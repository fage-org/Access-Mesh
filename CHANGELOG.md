...
- **定时任务读端点补权限门禁（T-ACCESS-054，安全收紧）**：`POST /api/access/job/detail`、`job/page`、`job/log/page` 三读端点此前**零权限校验**——任意租户内登录用户即可翻任务配置（invokeTarget/cron）与执行日志；现补 `ADMIN_JOB:VIEW` 类型级门禁（拒绝先于任何读取）。bootstrap 固定图同步补 `ADMIN_JOB` 最小运营三档类型级授权（VIEW/TRIGGER/ENABLE，不可转授）——此前固定图零 ADMIN_JOB 授权=新部署任务管理面全端点 403（种子对账任务的「按需手动触发/启用周期巡检」无正规入口）；CREATE/UPDATE/DELETE 维持无种子（前端零任务页、外部零消费）。存量已初始化库重启将因缺三行（无墓碑）fail-fast 拒启（固定图加行不自动补权，处置=按 rebuild-runbook 重建库）。写端点 `job/update`/`job/toggle` 同批改先门禁后存在性检查（与 `job/trigger` 及 T-ADMIN-029 公告族同形态——无权限统一 403，不再泄露 JOB_NOT_FOUND 与 403 的 id 存在性差异）。任务底座（sys_job 调度/租约接管/幂等执行键）经盘点保留：唯一 `@JobInvocable` 消费者为自动授权对账任务（T-PERM-073）+两系统维护调度器。
...
- **可编辑字段显式清空协议贯通（T-API-004）**：用户 `phone`/`email`、类型 `description`、服务 `basePath`/`description`/`extra`、接口映射 `extra`、perm 轨主体 `extra` 七端点补 `xxxClear` 显式清空标志（true=清空为 NULL，经 UpdateEntity 强制写列真实落库；统一协议见契约总册 §2.7）。此前这些字段表单清空后提交 null 被后端「null=不修改」跳过——返回 200 但值原样保留（F009 静默失败）；协议化后清空唯一通道=`xxxClear`，目标字段空串/纯空白一律 400（杜绝空串入库与 `uk_user_phone` 部分唯一索引空串撞车）。**破坏性收紧（U006 拍板 2026-09-24）**：①新值与 `xxxClear=true` 同传全端点 400 拒绝——`role/update` 与 `resource-entity/update` 既有 `extraClear` 的旧「Clear 优先于 extra（新值静默丢弃）」口径同批退役（前端表单公式从不同传真值+true，正常单用形态零变化）；②类型 `extraClear` 任何非 null 值拒绝 20044（type extra 含服务端管理键 managedMode/syncSourceService/grantOriginRole，不支持整体清空）；③服务 `save` 创建分支（服务不存在新建）携带任一清空标志拒绝 20044（新建字段省略即为空）；service `extraClear` 语义=撤销 `syncTypes` 同步白名单（该服务同步通道全拒，fail-closed、重新提交 extra 即恢复）。
- **公告受众改 typed IDs + JSONB 数组（T-ADMIN-029，破坏性）**：`notice/create|update|detail` 的 `targetUserIds` 由逗号分隔字符串改为 `targetType`（`ALL`/`USER`，缺省 `ALL`）+ `targetUserIds: number[]`（落库 JSONB 数字数组）——旧逗号串此前直接写 JSONB 列，多用户公告创建必然 SQL 失败（`"1,2,3"` 非法 JSON）。受众语义明确化：`ALL` 带 IDs 拒（10008）、`USER` 空列表拒（10008）、目标用户不存在/跨租户拒（10001）、`ORG` 值域拒绝（90001，预留未实现）；`my-notices` 按受众过滤（`ALL` 全员 / `USER` 数组含该用户，此前不按目标过滤——定向公告全员可见）；`read` 标已读前置可见性校验（草稿/撤回/非受众统一 10401，不泄露存在性）。消费者盘点=零真实消费（前端通知面板为模板 mock），无兼容层。
