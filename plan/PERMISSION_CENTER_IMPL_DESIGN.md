...
public interface UserRoleDomainService {
     ...
     *   3. 未命中 → DB 实时解析：
     *      a. 直接角色：user_role WHERE target_type=ROLE AND (valid_from<=now OR NULL) AND (valid_to>=now OR NULL)
     *      b. 分组角色：user_role WHERE target_type=GROUP → 递归展开子分组（path LIKE） → role_group_role
     ...
     * 查询 user_role + role_group_role 找到受影响的用户 ID 集合后逐一失效。
     */
...
public interface PermCacheDomainService {
    ...
    // ---- 角色权限快照缓存 ----
    /** key: perm:role:perm-snapshot:{tenantId}:{roleId} */
...
public interface PermissionConflictDomainService {
     ...
     * 触发异步通知（发 MQ EVENT_PERMISSION_CONFLICT 消息）。
     *
     * @return 过滤后的权限条目列表
     */
...
| 接口         | 路径                               | 说明                                  |
| ------------ | ---------------------------------- | ------------------------------------- |
| 单次鉴权     | `POST /v1/auth/check`              | 精确判定一个用户对一个资源+操作的权限 |
| 批量鉴权     | `POST /v1/auth/batch-check`        | 一次请求判定多个资源+操作组合         |
| 接口权限快照 | `POST /v1/auth/interface-snapshot` | 返回 gateway 消费的接口权限快照       |
| 接口单次判定 | `POST /v1/auth/interface-decision` | 按 serviceCode+method+path 判定       |
...
### 3.3 接口权限快照（gateway 消费）

#### 入参 / 出参

```java
/** POST /v1/auth/interface-snapshot */
...
public record InterfaceSnapshotResp(
    ...
    List<ApiPermissionEntry> allowedApis   // 允许访问的接口列表（不含条件权限）
) {}
...
sequenceDiagram
    ...
    alt L1/L2 快照命中
        ...
        Note over RPD: 联查 role_resource_permission + resource_api_mapping<br/>过滤 condition_id IS NULL（条件权限不进快照）<br/>冲突角色已在调用方过滤
...
public interface RolePermissionDomainService {
     ...
     *
     * 操作位继承（inherit_mask）：
     *   检查 (operation.binary_bit & operation.inherit_mask) 时，
     *   若授权的操作 effective=binary_bit|inherit_mask 包含目标操作位则匹配。
     *
     ...
     * 过滤 condition_id IS NULL 的授权（条件权限不进快照）。
     */
...
| 接口               | 路径                                         | 说明                                 |
| ------------------ | -------------------------------------------- | ------------------------------------ |
| 批量授权           | `POST /v1/role-permission/batch-grant`       | 为角色批量新增/更新/删除资源权限     |
| 查询角色权限       | `POST /v1/role-permission/list`              | 查询角色已有权限列表（含子权限展开） |
| 查询资源被授权情况 | `POST /v1/role-permission/resource-grantees` | 哪些角色拥有某资源的权限             |
...
```java
/** POST /v1/role-permission/batch-grant */
...
sequenceDiagram
    ...
    RDD-->>PS: supplementedAddItems（含自动补全的新增条目）
...
| 自动补全不重复             | `ResourceDependencyDomainService.autoGrant`                | 若角色已拥有依赖资源的权限则跳过                                              |
...
| 角色权限快照（资源+操作位） | `perm:role:perm-snapshot:{tenantId}:{roleId}`              | 60 秒     | 5 分钟               |
...
权限变更（role_resource_permission）
  ...
  → 关联用户角色缓存失效（evictEffectiveRoles for all users with this role）
...
### 5.3 角色级版本与 Gateway 版本校验流程

```
Gateway 持有：{ roleId → versionNo } 的本地映射（来自上次快照）

每次请求（或轮询）时：
  1. 从 Token 中拿到用户有效角色列表（网关首次需向 permission-center 拉取）
  2. 查询各角色当前 permissionVersion（L2 Redis）
  3. 与本地映射对比：任一角色版本发生变化 → 重新拉取 interface-snapshot
  4. 更新本地映射 + L1 接口快照缓存
```
...
// 接口单次判定
record InterfaceDecisionReq(Long tenantId, Long abstractUserId,
                             String serviceCode, String httpMethod,
                             String path, Map<String,Object> context) {}
record InterfaceDecisionResp(boolean allowed, String denyReason) {}
```
