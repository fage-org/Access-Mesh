...
**未交付清单**：租户开通/运营能力（首期固定单租户）；PERSONAL/GROUP_ROLE 角色生命周期（首期功能角色仅 BASIC_ROLE；GROUP_ROLE 写入口已随 T-PERM-043 删除，create/update/sync/full-sync 拒绝 20022、extra-roles/* 退役、前端选项隐藏，读模型保留冻结，存量行可经 delete/move 清理）；资源依赖页（3.4）真实联调（api 层已按契约对齐，未含于前端 Phase 3 联调批次，页面级联调与 mock 退役待排期，Q-011）；example 报表数据范围与动态数据权限（暂缓，等 PM 重申——T-PERM-035/036）。其余管理页面均已切换真实接口（前端 Phase 3 九任务 2026-09-04 收口——组织/用户、角色、资源+操作、权限授予、权限查询、条件/冲突规则、业务域、系统/服务配置与日志）；文件夹级授权已交付（T-ADMIN-025，ADMIN_FILE 文件夹实例 bootstrap 预置 + 上传惰性登记）。设计文档入口见 [docs/README.md](docs/README.md)。
