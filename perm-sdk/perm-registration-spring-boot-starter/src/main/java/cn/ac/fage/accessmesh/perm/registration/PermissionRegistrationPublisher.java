...
public final class PermissionRegistrationPublisher {
    ...
    public PermissionManifestReq capture(String generation, String revision, PermissionManifestProvider provider) {
        var manifest = new PermissionManifestReq(1,generation,revision,Objects.requireNonNull(provider).dependencies());
    ...
    public List<TenantRegistrationProvider.TenantPublication> capture(TenantRegistrationProvider provider) {
        ...
        var scopes = new java.util.HashSet<java.util.Map.Entry<Long,String>>();
        for (var publication : publications) {
            validation.validate(publication.manifest());
            if (!scopes.add(java.util.Map.entry(publication.target().tenantId(),publication.target().serviceCode()))) {
    ...
    public SyncResultResp publish(RegistrationTarget target, PermissionManifestReq manifest) {
        Objects.requireNonNull(target); validation.validate(manifest);
        var response = client.fullSync(target.tenantId(),target.serviceCode(),target.credentialId(),target.credentialSecret(),manifest);
