 ...
 * T-FE-015 起侧栏菜单切换为后端派生（/api/access/api/access/auth/user-menu menus 树直接渲染，
 * 标题/图标/层级/排序来自 sys_menu，可见性 = v3.5 §4.1 ∃op 派生）：
 * <ul>
 *   <li>登录路径：loginByUsername 已拉取 user-menu，此处复用 store 内存结果不重复请求</li>
 *   <li>已登录 F5/启动（menus 内存态丢失）：重取 /api/access/api/access/auth/user-menu；失败 fail-closed 空菜单 +
...
async function initRouter() {
  ...
  if (getToken() && userStore.menus.length === 0) {
    try {
      ...
      console.warn("[initRouter] failed to load /api/access/api/access/auth/user-menu", err);
    }
  }
  handleAsyncRoutes([]);
