 ...
 * （`/api/perm/resource-entity/*` + `/api/perm/operation-permission/*`）；
 * Phase 1 由 mock/resource-operation.ts（vite-plugin-fake-server）提供假数据。
 ...
 * 后端实现：permission-center ResourceController + OperationController
 *
...
/** 查询资源树（POST /api/perm/resource-entity/tree）。
...
/** 查询资源列表（POST /api/perm/resource-entity/list，分页）。
 *  本页以树为主不消费，保留对齐契约。 */
...
/** 查询资源详情（POST /api/perm/resource-entity/detail，IdReq{id}）。
...
/** 创建资源（POST /api/perm/resource-entity/create） */
...
/** 更新资源（POST /api/perm/resource-entity/update）。
...
/** 移动资源（POST /api/perm/resource-entity/move）。
...
/** 删除资源，支持批量（POST /api/perm/resource-entity/remove，IdsReq{ids}）。
 *  🔧 用内部 id，Phase 2 切业务键（T-PERM-028）。 */
...
/** 查询操作权限列表（POST /api/perm/operation-permission/list）。
...
export const getOperationList = async (
  ...
  >("post", "/api/perm/operation-permission/list", { data: params });
  return unwrap(res);
};

/** 查询操作权限详情（POST /api/perm/operation-permission/detail，IdReq{id}）。
 *  🔧 用内部 id，Phase 2 切业务键（T-PERM-028）。 */
...
/** 创建操作权限（POST /api/perm/operation-permission/create） */
...
/** 更新操作权限（POST /api/perm/operation-permission/update）。
...
/** 删除操作权限，支持批量（POST /api/perm/operation-permission/remove，IdsReq{ids}）。
 *  🔧 用内部 id，Phase 2 切业务键（T-PERM-028）。 */
