...
/**
 * 查询角色权限配置（POST /api/access/role-resource-permission/list）。
...
/**
 * 聚合授权提交（POST /api/access/role-resource-permission/apply-grant-plan，唯一写入口）。
...
/**
 * 查询子权限允许类型（POST /api/access/role-resource-permission/sub-perm-allowed-types）。
