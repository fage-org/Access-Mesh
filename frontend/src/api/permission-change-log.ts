...
// ========== diff_snapshot 结构化类型（对齐 api-contract 总册 §16.4 diff_snapshot 规范（原册 §6.8）） ==========

/** 事件类型固定枚举（api-contract 总册 §16.4 diff_snapshot 规范（原册 §6.8）） */
...
/** 变更类型固定枚举（api-contract 总册 §16.4 diff_snapshot 规范（原册 §6.8）） */
export type DiffChangeType = "ADD" | "REMOVE" | "UPDATE";

/** 权限项业务键（总册 §16.4 diff_snapshot 规范：domainCode+resourceTypeCode+resourceCode+codeType+operationCode+scopeMode） */
...
/** 角色来源业务键（总册 §16.4 diff_snapshot 规范） */
...
/** 资源业务键（总册 §16.4 diff_snapshot 规范（原册 §6.8）RESOURCE_STATUS_CHANGE 示例） */
...
/** diff_snapshot.items[] 单项（总册 §16.4 diff_snapshot 规范） */
...
/** diff_snapshot 顶层结构（总册 §16.4 diff_snapshot 规范：eventType + items[]） */
...
export type ChangeLogResp = {
  ...
  /** 结构化变更摘要 JSON 字符串（总册 §16.4 diff_snapshot 规范（原册 §6.8），可空）。前端 JSON.parse 后取 eventType + items[] */
...
/** 查询权限变更日志列表（POST /api/access/log/change/list，契约路径已随 T-PERM-032 修正）。
...
/** diff items[].changeType 合法枚举（api-contract 总册 §16.4 diff_snapshot 规范（原册 §6.8）），超出此集合的元素视为非法并过滤 */
 ...
 *  契约 总册 §16.4 diff_snapshot 规范（原册 §6.8） 七枚举之一（T-PERM-032 增补），EVENT_TYPE_META 有正式条目；未知值仍 fallback 显示原值。 */
