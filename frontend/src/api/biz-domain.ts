 ...
 *
 * 契约依据：docs/design/access-service-api-contract.md §14.1（biz-domain，T-PERM-026 收口契约要点）
...
/** 查询业务域列表（POST /api/access/biz-domain/list，BizDomainListReq）。
...
/** 查询业务域详情（POST /api/access/biz-domain/detail，BizDomainDetailReq{domainCode}）。
...
/** 创建业务域（POST /api/access/biz-domain/create，BizDomainCreateReq）。 */
...
/** 更新业务域（POST /api/access/biz-domain/update，BizDomainUpdateReq）。 */
...
/** 删除业务域（POST /api/access/biz-domain/remove，IdsReq 批量软删）。
