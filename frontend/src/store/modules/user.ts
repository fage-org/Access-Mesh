...
export const useUserStore = defineStore("pure-user", {
  state: (): userType => ({
    ...
    /** 后端下发的菜单树（v1.4 双轨并行：菜单可见性轨道，登录后由 /api/access/api/access/auth/user-menu 填充） */
    menus: [],
    // 是否勾选了登录页的免登录
    isRemembered: false,
    // 登录页的免登录存储几天，默认7天
  ...
  actions: {
     ...
     *  3. 调 `/api/access/api/access/auth/user-menu` 拿 menus + roles + permissions，写 Pinia + localStorage
    ...
    async loginByUsername(data: LoginFormData): Promise<LoginResp> {
      ...
      try {
        ...
        console.warn("[loginByUsername] failed to load /api/access/api/access/auth/user-menu", err);
      }
      return loginData;
    },
    /**
