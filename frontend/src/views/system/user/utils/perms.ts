 ...
 *
 * 双轨：
 * - 轨道 1（菜单可见性）：`MENU:VIEW` —— 决定路由可达，不在本表登记
 * - 轨道 2（按钮权限）：本表所有条目 —— 决定页面内按钮是否显示
 *
 * 详见 `docs/design/org-user-permission-contract.md` v1.4 + §4 权限矩阵。
 *
 * ## 「岗位 = 特殊组织」
 * 岗位与普通组织共用 `/org/*` 端点（通过 `orgType=2` 区分），资源类型同为 `ORG`，
 * 但乙层使用**独立的精化操作码**实现"组织管理员 ≠ 岗位管理员"细粒度配权：
 *   - 岗位查看      → `ORG:VIEW_POSITION`
 *   - 岗位 CRUD     → `ORG:CREATE_POSITION / UPDATE_POSITION / DELETE_POSITION`
 *   - 岗位用户挂载  → `ORG:ASSIGN_POSITION_USER`
 * **不新增 POSITION 资源类型**，以保持锚点单一性、避免 user-org 关系双写。
 */
export const ORG_USER_PERMS = {
  ...
  // 功能角色分配 = ROLE:MANAGE（目标角色实例），不得复用配权语义操作码（已删除的 ADMIN_ROLE:GRANT/REVOKE，属红线）。
