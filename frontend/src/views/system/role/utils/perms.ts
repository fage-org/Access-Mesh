 ...
 *
 * ## 权限锚点（B1：前端操作码口径对齐后端）
 * 资源类型 `ROLE`（access-service 乙层模型），操作码对齐 OperationCode。
 ...
 * GROUP_ROLE 面板与 `ROLE:ASSIGN`/`ROLE:REVOKE` perm 键已随 2026-09-14 轻量清扫批次
 * 删除（git 历史可恢复）；未来 role_inclusion 单事实源立项时随面板恢复接线。
 *
 * ## 范围限定
