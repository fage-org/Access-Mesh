...
- **角色互斥授权时校验**（T-PERM-063，2026-09-12 定案）：`user-role/assign`、`batch-assign` 写路径事务内校验「授予后有效角色集」（现有效∪本批新增、仅计启用角色），命中 ROLE_MUTEX 对整批原子拒绝 20062；conflict-rule create/update 的 ROLE_MUTEX 分支存量双持非空拒绝 20063（候选经 `findUserIdsByEffectiveRoles` 反查含组角色间接持有）；互斥规则读取写路径 DB 直查不经 ROLE_MUTEX_RULE 缓存；后续新增用户-角色写入口必须挂同款守卫（user_role 直授是唯一持有通道）。
