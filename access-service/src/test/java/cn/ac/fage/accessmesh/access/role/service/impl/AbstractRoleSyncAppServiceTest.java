...
class AbstractRoleSyncAppServiceTest {
    ...
    void shouldRejectGroupRoleTypeOnFullSync() {
        mockHeaderMatch();
        cn.ac.fage.accessmesh.access.sync.dto.AbstractRoleFullSyncReq req =
                new cn.ac.fage.accessmesh.access.sync.dto.AbstractRoleFullSyncReq(
                        new cn.ac.fage.accessmesh.access.sync.dto.AbstractRoleSyncScope(
                                SOURCE_SERVICE, "GROUP_ROLE", "ROOT"),
                        java.util.List.of(new cn.ac.fage.accessmesh.access.sync.dto.AbstractRoleSyncItem(
    ...
    void shouldDefaultParentTypeToScopeTypeWhenOmitted() {
        ...
        cn.ac.fage.accessmesh.access.sync.dto.AbstractRoleSyncItem omittedType =
                new cn.ac.fage.accessmesh.access.sync.dto.AbstractRoleSyncItem(
    ...
    void shouldKeepOldGraphEdgeWhenNameOnlyUpdatePrecedesCycleClose() {
        ...
        cn.ac.fage.accessmesh.access.sync.dto.AbstractRoleSyncItem nameOnly =
                new cn.ac.fage.accessmesh.access.sync.dto.AbstractRoleSyncItem(
    ...
    private cn.ac.fage.accessmesh.access.sync.dto.AbstractRoleSyncItem item(
            String externalId, String parentTypeCode, String parentExternalId, long seq) {
        return new cn.ac.fage.accessmesh.access.sync.dto.AbstractRoleSyncItem(
                externalId, externalId, parentTypeCode, parentExternalId, 1, 0, null,
                "org", externalId, new SyncVersionRef(OCCURRED_AT, seq));
    }

    /** 测试夹具：full-sync 请求 */
    private cn.ac.fage.accessmesh.access.sync.dto.AbstractRoleFullSyncReq fullSyncReq(
            java.util.List<cn.ac.fage.accessmesh.access.sync.dto.AbstractRoleSyncItem> items) {
        return new cn.ac.fage.accessmesh.access.sync.dto.AbstractRoleFullSyncReq(
                new cn.ac.fage.accessmesh.access.sync.dto.AbstractRoleSyncScope(
    ...
    private void stubNewerExistingMetadata(String externalId) {
        ...
        when(syncMetadataDomainService.isNewerVersion(
                org.mockito.ArgumentMatchers.<cn.ac.fage.accessmesh.access.sync.metadata.SyncMetadata>argThat(
                        m -> m != null),
