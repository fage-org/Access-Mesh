 ...
 * 不含任何 /sync-task 或 /audit-log 映射（含非注解注册通道），且经完整请求链
 * 实际请求返回 404「不存在」响应（而非 401/403/500 等其他语义）。
...
class RetiredSyncTaskEndpointTest {
    ...
    @Test
    @DisplayName("注册表负向：全量 Context 无任何 /sync-task 或 /audit-log 映射")
     ...
     * 匿名请求先被 RequestContextInterceptor fail-closed 401（不向匿名调用者泄露路径存在性），
     * 已认证请求则 404。两者都是「无法提供服务」的明确负向结果；
    ...
    private static org.springframework.test.web.servlet.ResultMatcher retiredPathRejected() {
        return result -> assertThat(result.getResponse().getStatus())
            .as("退役路径匿名请求必须被拒（401 fail-closed 或 404），不得命中业务处理器")
            .isIn(401, 404);
    }
