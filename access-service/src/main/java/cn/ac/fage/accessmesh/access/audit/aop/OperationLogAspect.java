...
public class OperationLogAspect {
    ...
    /** 登录会话中操作者名称键（AuthAppServiceImpl 登录成功写入，本切面读取回填） */
    ...
    private void recordLog(OperationLog opLog, ProceedingJoinPoint joinPoint, Object result, int costTime) {
        ...
        // 操作者名称从登录会话读取（AuthAppServiceImpl 登录成功写入）；未登录/无会话调用为 null
    ...
    /**
     * 从登录会话读取操作者名称（AuthAppServiceImpl 登录成功写入 {@link #OPERATOR_NAME_SESSION_KEY}）。
