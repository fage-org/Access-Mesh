...
import cn.ac.fage.accessmesh.access.user.dto.resp.MemberCandidateItemResp;
import cn.ac.fage.accessmesh.access.user.dto.resp.ResetPasswordResp;
...
public class UserAppServiceImpl implements UserAppService {
    ...
    public UserResp getUser(Long id) {
        ...
        // P1-2 修复：组织可见性裁剪。pageUsers 已按 OrgVisibilityQueryAppService 限制可见用户，
     ...
     * <p>
     * 默认树范围解析走领域共享入口（claude 外评 P3 类推收敛——与 UserWriteAppServiceImpl
     * 的逐字副本同源，Q-025 同款收敛；无默认配置/根失联时共享入口返回空列表，
     ...
     * 与跨域查询服务（UserRoleQueryAppServiceImpl）的组织/岗位角色类型映射同语义。
     */
