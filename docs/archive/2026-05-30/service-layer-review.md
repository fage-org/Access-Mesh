...
### 统一权限查询引擎（PermQueryEngine）重构收益

| 指标 | 重构前 | 重构后 |
|------|--------|--------|
| 权限查询路径 | 5条独立链路 | 1个 Engine |
| `PermissionServiceImpl` 代码量 | ~1923行 | ~841行 (-56%) |
...
| `checkInterface()` | 230行 | 25行 | -89% | ✅ |
| `queryResources()` | 55+280行 helpers | 17行 | -95% | ✅ |
| `queryScopes()` | ~200行+helpers | ~50行 | -75% | ✅ |
