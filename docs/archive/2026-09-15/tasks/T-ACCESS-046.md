...
codex sol 外评轮（2026-09-15，gpt-5.6-sol@xhigh、read-only、禁子代理，同范围）：P0-P2 零、P3×1 且判「需补救修复」——归档计划声称的「createUser-带-org 链路既有行为锁」经核实不存在（全仓 6 处 UserCreateReq 构造 orgId 均 null；LocalProjectionDomainServiceImplTest 以 mock TypeResolutionService 只锁组件级语义；UserRoleWriteProjectionPgIT 走权限轨 createUser 无 orgId 分支）。补救落地：新增 `UserCreateWithOrgPgIT`（真实 UserWriteAppService.createUser(orgId) 全链，真实 TypeResolutionService 读回 + sys_user/abstract_user/sys_user_org/user_role 四表断言，org 门禁经 @MockBean 放行先例）——读回链若加结果缓存/REQUIRES_NEW/调序即 20046 整体回滚、用例必红。单测 1/1 绿（2026-09-15，t046_r06it2.log）。三轮外评（计划 claude+grok / 落地 claude+grok / 复评 codex sol）至此全闭环。
