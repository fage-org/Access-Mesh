...
| 权限条件选择器 + 规则编辑器 | 条件管理（T-FE-009）+ 权限授予附加设置（T-FE-014） | ✅ 已确认抽取 ReConditionPicker/ReConditionEditor | T-FE-024 |
| 资源树选择器 | 权限授予 + 资源管理 + 权限查询 | 📋 登记数据适配层，不抽整块 UI（语义差异大） | - |
| 角色选择器 | 权限授予 + 功能角色分配 + 角色管理（父角色选择） | 📋 登记数据适配层，不抽整块 UI（语义差异大） | - |
| Diff 对比面板 | 权限变更日志 + 操作日志详情 | ❌ 暂不抽取（T-FE-005 未扩展 diffSnapshot，未达 2 页；待扩展后抽 ReDiffViewer） | - |
| 分页表格 hook | 类型定义/系统配置/操作日志/变更日志（4 页） | 📋 待 Phase 4 评估（达 2+ 页但各页已稳定，抽取是重构） | - |
| JSON 编辑校验 | 系统配置/业务域/权限条件（3 页 JSONB 字段） | 📋 待 Phase 4 评估（组件简单低优先级） | - |
