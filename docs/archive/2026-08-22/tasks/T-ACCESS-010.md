...
- **AI 评审修复（2026-08-22，1 P1 + 3 P2 全部属实并修复）**：
  - **P1 Starter 未装配拦截器**：`@EnableFeignClients` 只注册 Feign 接口，业务服务组件扫描覆盖不到 SDK 包，`FeignInternalSyncInterceptor` 从未被装配（SDK 调用缺 X-Internal-Secret/X-Service-Code 会被服务端拒绝，且 perm.service-code fail-fast 不生效）。修复：`PermClientAutoConfiguration` 显式 `@Import(FeignInternalSyncInterceptor.class)`（类上 `@ConditionalOnProperty(perm.internal-secret)` 在 @Import 下照常评估）；新增 `PermClientAutoConfigurationTest`（ApplicationContextRunner，4 项：装配成功含字段注入断言 / 未配置 internal-secret 不注册 / 缺 service-code 启动失败 fail-fast / client.enabled=false 停用）。**附带发现**：①测试上下文无 `PropertySourcesPlaceholderConfigurer` 时未解析占位符以字面量静默注入不报错——runner 显式注册 `PropertyPlaceholderAutoConfiguration` 复现真实 Boot strict 语义；②starter 模块 `spring-boot-starter` 未排除默认 logging，与 common 传递的 log4j2 形成桥接环（log4j-to-slf4j ↔ log4j-slf4j2-impl）导致自动配置基础设施初始化失败——按 access-service/gateway 同模式排除（此前无 Spring 上下文测试从未暴露）。
  - **P2 契约测试不封闭**：原实现按 @PostMapping 预过滤（新增 GET/未标注方法逃逸）、@RequestBody 仅 anyMatch（未证明唯一参数）、Set 比对掩盖重复路径。重写为封闭语义：全部实例方法（不预过滤）必须 @PostMapping（禁 GET/PUT/DELETE 混入）、恰好 1 参数且标注 @RequestBody、无 Spring Web 参数类型（@RequestParam/@PathVariable/@RequestHeader）、value 恰一且 path/params/headers 为空、方法总数=清单数、路径 List vs Set 计数防重复。
  - **P2 auth-routes 行为未固化**：`GatewayApplicationConfigTest` 补强——路由总数恰为 3（防增删漂移）、auth-routes 断言 Path=/auth/** + StripPrefix=0、example-service 断言 Path=/example/** + StripPrefix=1（原路径行为完整回归保护）。
  - **P2 旧名清理遗漏**：`PermissionFilter` 两处生产告警 "Permission-center unreachable" 改 "Access-service unreachable"；内部异常类 `PermCenterUnreachableException` 改名 `AccessServiceUnreachableException`（仅本文件引用，异常名将出现在日志堆栈）；`.github/copilot-instructions.md` 清理 4 处——sourceService 示例 admin-service→example-service（原示例是被服务端拒绝的退役身份，误导生成代码）、错误码分段表改管理域/权限域措辞、commit scope 示例 `feat(permission-center)`→`feat(access)`。
  - **回归**：全量 `mvn test` BUILD SUCCESS——access-service 610（37 skip 同基线）、gateway 59、perm-client-starter 12（新增装配测试 4 项）；gateway/perm-sdk 残留终扫仅剩负向断言提示文本与归并历史说明。
  - **二轮复评 2 P2 + 1 P3 全部属实并修复**：①契约测试禁用注解断言原检查参数类型（`getParameterTypes`）无效——DTO 参数同时带 `@RequestBody @RequestParam` 仍通过，改为直接遍历 `parameterAnnotations` 拒绝 RequestParam/PathVariable/RequestHeader 注解实例；②路由总数断言原基于 `byId` HashSet（重复 ID 被折叠），改为直接断言 `routes.size()==3` + `byId.size()==3` 组合（定义数恰 3 且 ID 无重复）；③删除重构遗留的未使用 `Collectors` import。回归通过（gateway 59 / perm-client-starter 12）。
