...
acceptance:
  ...
  - "测试覆盖注解契约、独立日志写入失败异常传播与调用方兜底、敏感字段脱敏；主事务回滚/独立日志失败的验证边界为上述可测单元（不补 Spring 集成事务测试，REQUIRES_NEW 事务语义属第三方框架行为，见非目标）"
design_writeback:
  required: true
  status: done
...
- **租户解析**：`OperationLogAspect` 解析租户优先级为 ① `OperationLogRuntimeContext.setTenantId`（方法体内显式登记，覆盖匿名认证派生端点——OAuth2 token/refresh/revoke 从授权码/刷新令牌/JWT 载荷解析租户后登记）；② 方法参数 `tenantId`（如登录失败自动锁定 `lockUser`）；③ `TenantContextHolder`。三者皆空的纯匿名端点该条操作日志跳过并告警（不写 `tenant_id = null` 违反 NOT NULL）。OAuth2 审计：`@OperationLog`（`OAUTH2_TOKEN_ISSUE` / `OAUTH2_TOKEN_REFRESH` / `OAUTH2_TOKEN_REVOKE`，targetType=`oauth2_token`）经 runtime override 正常落库；token 签发/刷新（成功与失败）由 `sys_login_log`（loginType=OAUTH2）承载登录审计——成功写 status=1，失败（凭据/授权码/回调/PKCE/刷新令牌无效等）在租户可解析时写 status=0、解析不出租户时跳过并告警；revoke 仅 `@OperationLog`、无登录日志。
...
- **permission 覆盖**：`AppServiceOperationLogCoverageTest` 包扫描 `permission.service.impl` 的 public `@Transactional` 非 readOnly 方法必须有 `@OperationLog`；已标注方法校验 module / action / targetType / SpEL。admin/application 强制全覆盖见 [T-ACCESS-014](../../2026-08-27/tasks/T-ACCESS-014.md)。
...
- **不补 Spring 集成事务测试**：主事务回滚、REQUIRES_NEW 独立日志真实事务语义属第三方框架行为，本任务以注解契约 + 异常传播 + 调用方兜底单元测试覆盖可测边界（验收对照第 7 行）。
- `sys_menu` DDL-实体漂移留待 T-ACCESS-012。
- admin/application 域 `@OperationLog` 强制全覆盖： [T-ACCESS-014](../../2026-08-27/tasks/T-ACCESS-014.md)。
