...
| T-ADMIN-020 | access-service admin 域 CRUD 代码清理（✅ done 2026-09-12：规范清单重排查零可清扫描项，零代码变更定性收口） | admin-service | 🟢 低 |
...
- 2026-09-12：T-FE-023 done（范围定案见 registry 同日行：指南全景落 design 根 + 补自有类型链路用例）——extension-guide.md 成文（待确认评审稿）+ CustomResourceTypeSlicePgIT 全绿；三处事实修正进指南（创建类型预置 CRUD 四操作/新类型首笔授权部署方种子引导 §3.5/本地用户主体类型 LOCAL_USER）。T-PERM-039 done——覆盖重估（157 测试文件 vs 附录 B 27+；permission 域 33/36 有直接测试）+ 旧「每接口 1+2」口径定性收口。
- 2026-09-12：T-ADMIN-020 done（范围定案见 registry 同日行：重核对后清扫）——规范清单重排查（残留/端点形态/Date/禁库/命名/异常/死代码/日志输出/操作日志/N+1 启发式/DTO 校验）零可清扫描项，零代码变更定性收口；原痛点 #6 口径（185 文件零测试等）漂移登记任务卡。**Phase 4 四项任务全部 done**；收口全量回归 Reactor 全绿（含 E2E）+ 双轨评审处置完成。
