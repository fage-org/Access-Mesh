...
- 2026-09-06 后续实测注记（`-T` 模块并行试验，用户要求试用）：全仓 `mvn test -T 1C` **证伪未采纳**。第二轮全绿墙钟 8:45（access-service 4:14 + gateway 4:16 纯串行相加），对比串行基线 522s 无增益反略亏——根因是 `gateway/pom.xml` 对 `access-service`、`example-service` 的 **test-scope 模块依赖**（E2E 子进程启动需要其类路径），reactor 依赖边把 gateway 完整排在 access-service 之后，两大重模块（各 ~4min）无法并行，`-T` 仅并行 ~27s 小模块且被负载抖动（重模块各 +6-8%）抵消。附带发现：`TaskExecutionLeaseConcurrencyTest.sameInstanceTakeoverFencedByAttempt` 在 -T 第一轮负载下失败 1 次（sleep 1200ms 对 1s 租约仅 200ms 余量，第二次抢占被判未过期返回 null），隔离复跑 10/10 绿、第二轮全量绿——串行口径不受影响；若未来重启模块并行需先拉长余量（sleep 加大或轮询至可抢占）。后续墙钟杠杆在 E2E 分轨与结构性把 E2E 拆出 gateway 模块，当时未实施待口径。
