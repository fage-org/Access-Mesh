...
- 2026-08-24 执行：T-ACCESS-021 自动化轨完成——跨服务 E2E IT 落位 gateway 测试域（access-service 为 test 依赖、双服务子进程随机端口 + PG/Redis Testcontainers、免 Nacos 静态实例直连、Redis 读码、子进程级重启、fail-closed 与⑦合并、步骤⑤ IT 内经 apply-grant-plan 同构请求），固定 8 步两轮全绿（8/8，77s 级）；gateway 单测 83 + access-service 单测 695 全绿。E2E 揪出并修复两处真实产品缺陷：resource-api-mapping/create 缺省 matchOrder 显式 null 写库违例（缺省 0 对齐 DDL）、用户创建 status 两侧不同源（未传 status 时 sys_user 启用而 abstract_user.enabled=false，主体被禁用致快照恒空全接口 403；缺省解析一次同源，DDL 权威语义 1=启用，DTO javadoc 与 admin 契约 create 段错误表述同步更正）。GUI 段（授权页授予+截图）环境与①-④预备数据已就绪，因 IAB 点击通道环境故障经用户决策存档 runbook、重启后重试；README 回写与任务收口随 GUI 段完成后进行。八项执行决策与证据详见任务卡。
