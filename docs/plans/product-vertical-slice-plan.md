...
- 2026-08-23 评审修复：T-ACCESS-019 经 codex gpt-5.6-sol 只读评审（4 P1 + 1 P2 全部核实属实）——P1-1 name=null 投影 NOT NULL 回归（调用侧 externalId 兜底）；P1-2 禁用主体仍可通过鉴权（存量缺口，用户拍板本任务内修：resolveEffectiveRoles 批量过滤 abstract_user.enabled，禁用者有效角色置空）；P1-3 move/delete 角色受影响用户反查在提交后沿当前树漏旧链成员（findUserIdsByEffectiveRoles 事务内预计算 + markUsers，invalidateRoleCacheByRoles 重构复用同口径）；P1-4 删除路径补 local-projection 逐角色 DELETE 日志（覆盖级联子孙全量）；P2 补 moveRole 单测 + IT 父镜像/fail-closed/禁用拒鉴用例。修复后 673 tests 0 failures（IT 6 用例）。
- 2026-08-23 二轮评审修复：T-ACCESS-019 经 codex gpt-5.6-sol 二轮只读评审（2 P1 + 2 P2）——P1-A 禁用 GROUP_ROLE 仍授出展开（存量缺口，按一轮同类拍板延伸本任务内修：resolveGroupRolesBatch/expandInMemory 禁用剪枝）；P1-B① 组角色直绑成员反查漏覆盖（findUserIdsByEffectiveRoles 补 GROUP_ROLE 直绑 + relation 行 null 过滤）；P1-B② extra.basicRoleIds 反查经核实零生产者不修（登记遗留）；P2-A 投影软删补 code_type=default 对称限定；P2-B 投影日志改传已解析 operatorId；新增 IT 组角色生命周期用例（预热缓存 → 禁用失权 → 级联删除投影）。二轮另发现 GROUP_ROLE extra 双轨不一致存量问题（relation 行不参与授权展开、extra JSON 无写入方）登记遗留归 T-PERM-043 裁决。修复后 674 tests 0 failures（IT 7 用例）。
