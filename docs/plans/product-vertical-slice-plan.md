...
- 2026-08-23 执行：T-ACCESS-019 done——LocalProjectionDomainService 新增 ROLE/USER 资源投影四方法（upsertRoleResource 父镜像 fail-closed、softDelete 批量 owner 过滤且限定 code_type=default、upsertUserResource code=subjectId）；RoleManage 四写路径与 UserManage 三写路径同事务投影 + @PermissionChange/mark 补齐 + permission_change_log 登记，abstract-user/update 门禁升实例级；move/delete 受影响用户事务内预计算失效（findUserIdsByEffectiveRoles ≤4 SQL，invalidateRoleCacheByRoles 复用同口径并覆盖 GROUP_ROLE 直绑）；鉴权可用性收口（禁用主体有效角色置空、组角色仅 status=1 展开并剪枝非启用嵌套组）；管理入口保留清单换值 {USER, ORG, MENU, ROLE}；修复存量 bug SubjectDomainServiceImpl.selectValidRoleById 实参反转（updateRole/moveRole/deleteRoles 对 roleId≠租户号恒报不存在）。UserRoleWriteProjectionPgIT 7 用例（真实 PG+Redis：写路径产投影 → 实例门禁按业务编码命中/拒绝 → 禁用/组角色失权 → 投影故障注入整体回滚）。全量 674 tests 0 failures。遗留：外部 sync 三入口无投影/无缓存失效、GROUP_ROLE extra 双轨不一致（归 T-PERM-043 裁决），见任务卡。T-ACCESS-020 依赖解除。
