...
- 2026-08-22：T-ACCESS-011 done（契约/回滚/架构/空库/双实例验收门禁：全量回归 779 tests 0 失败——access 633 / gateway 74 / common 60 / perm-client 12，37 skipped 全部 Docker 门控待 CI）。新增验收测试 38 个：路径快照契约（198 路径精确比对 + 归并前后 210→198 diff 全部有设计背书 + POST-only/无路径参数/@RequestParam 仅文件上传例外）、错误码扫描（归并前 53+37 基线固化逐码保持 + 分段独占/无 4xxxx/跨枚举无重复）、退役端点注册表+运行时负向、Gateway fail-mode 删除证明（字段/活跃键/指标三件套）、会话生命周期（真实 /auth/login 签发 + 绝对/空闲失效缩短配置语义验证 + 注销 + OAuth2 客户端自定义 TTL 双路径）、Gateway AuthTokenFilter 11 用例。**发现并修复 P0**：AuthTokenFilter getExtra 在 sa-token 1.38 默认 StpLogic 恒抛 ApiDisabledException 致所有合法令牌恒 401（旧仓库遗留，非归并引入），修复改读 SaSession（tenantId/subjectTypeCode/operatorName）+ 修复 operatorName 键名错配与无效令牌 NPE→500 两处次生缺陷（用户决策「改读 SaSession」）。4 项用户决策：Docker 缺失按遗留待 CI+现状降级 / 契约回归用路径快照测试 / 错误码固化归并前基线+规则断言 / 会话失效缩短配置验证语义。遗留：37 个 Testcontainers 测试待 CI（P1，用户决策放行）、api-contract §5 清单欠账 2 条移交 T-ACCESS-012。详见任务卡完成记录。
