...
- 2026-08-22：T-ACCESS-011 done（契约/回滚/架构/空库/双实例验收门禁：全量回归 783 tests 0 失败——access 635 / gateway 76 / common 60 / perm-client 12，37 skipped 为 Docker 门控，按环境受限豁免待 CI（计划归档前必须 CI 跑绿））。新增验收测试 40 个：路径+签名快照契约（198 路径 + 198 条路径→请求体/响应类型签名 + POST-only 按 @PostMapping 注解身份判定 + 全映射值枚举 + 统一响应 PermResult 包装断言 + 归并前后 210→198 diff 全部有设计背书）、错误码扫描（归并前 53+37 基线固化逐码保持 + 分段独占/无 4xxxx/跨枚举无重复）、退役端点注册表+运行时负向、Gateway fail-mode 删除证明（嵌套叶子字段/活跃配置键/指标注册表三件套，含 fallback 计数器存在性）、会话生命周期（真实 /auth/login 签发 + expiresIn 单一来源断言 + 绝对/空闲失效缩短配置语义验证 + 注销 + OAuth2 客户端自定义 TTL 双路径）、Gateway AuthTokenFilter 13 用例。验收期修复两项：P0——AuthTokenFilter getExtra 在 sa-token 1.38 默认 StpLogic 恒抛 ApiDisabledException 致所有合法令牌恒 401（旧仓库遗留，非归并引入），改读 SaSession（tenantId/subjectTypeCode/operatorName）并修复 operatorName 键名错配与无效令牌 NPE→500；P1——Gateway 未执行无操作超时校验/续期与 access-service 口径不一致（闲置冻结令牌被放行注入可信头、仅用 example-service 的活跃用户被误冻结），补冻结 401 + 每请求滑动续期（纯 dao API，WebFlux 安全）。遗留：37 容器测试待 CI（豁免口径）、api-contract §5 清单欠账 2 条与 DTO 字段级封闭限制移交/登记。详见任务卡完成记录。
