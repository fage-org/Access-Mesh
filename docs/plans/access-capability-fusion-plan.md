...
- 2026-09-13：T-ACCESS-035 收口——四项死字段签名级消减（createLocalUserSubject/upsertAdminUser/UpsertUserKey 去 extraJson、upsertAdminOrg 去 sortOrder+extraJson；extraUsername/extraOrgType helper 与 bootstrap 两处字面量删除；MenuAppServiceImpl 死注入 + 两类死 import 清除）；实施期实证 flex 全列插入落显式 NULL 非列默认；顺带收益：admin 轨用户更新不再覆写 permission 域 req.extra（COALESCE/忽略 null 保留，BatchSqlIT 锁行为）；schema 三处注释同步；双轨评审代码轨 P3×2+文档轨 P2×2+P3×3 全处置；全量回归 -T 1C 含 E2E 绿。存量观察归 036：upsertAdminMenu 的 sortOrder 参数 impl 零消费。
