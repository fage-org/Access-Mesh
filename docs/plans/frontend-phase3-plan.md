...
| ID | 联调范围 | depends_on |
|---|---|---|
| T-FE-015 | 组织与用户（已实现 2.1；含 user-menu menus 轨道前端接线——登录链路切真实接口后菜单可见性切 v3.5 §4.1 ∃op 派生，T-PERM-037 收口归入 2026-08-31；接线含会话恢复——F5/启动重取 user-menu、失败 fail-closed 空菜单+可重试、不持久化不回退全量静态菜单） | T-PERM-037 |
