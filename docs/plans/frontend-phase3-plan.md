...
| T-FE-037 ✅ | 权限授予（4.1）- 组织联调（二期；主体树 status=1 仅启用 + 入口=组织卡片按钮/岗位行操作，定案见任务卡；已收口 2026-09-04，终态见任务卡完成记录） | T-FE-018, T-ADMIN-021 |
...
- 2026-09-04：**T-FE-037 评审修正收口**：preset 同主体判定改双字段比对 `isSamePresetSubject`（跨入口 id 碰撞防授权目标静默错乱，spec +4 用例、vitest 209/209）；「组织入口二期」现在时残留清扫（types/perms/index.vue/perm.ts/role-manage.md）+ §13.2 决策 4 修订标 + user 页 mock 时代注释；多标签直切不一致登记任务卡已知边界、停用入口按钮/SSOT 归属登记任务卡后续登记、ORG:VIEW 缺失占位补冒烟（测试角色仅 ROLE:VIEW 投影实测，左栏占位+零网络请求，测试数据全清）。
- 2026-09-04：**T-FE-037 收口（Phase 3 第九个即最后一个联调任务 done，API+浏览器双冒烟全过；后端零改动）**：授权页组织入口真实适配——SubjectTreePanel 组织分支（getOrgTree includePositions+status=1 一体树、ORG:VIEW 探查占位、岗位 tag）+ buildOrgSubjectTree 纯函数（kind 扩 ORG/POSITION，roleExternalId=String(sys_org.id)）+ hook preset/文案类型感知；入口两处（组织与用户页组织信息卡片按钮 + 岗位管理 Tab 岗位行操作，门禁 ROLE:VIEW）；主体树 status=1 仅启用（对齐角色 enabledOnly 先例，接受停用父组织下启用子树不可见边界，定案见任务卡）。冒烟：API 级 ORG/POSITION 双主体 list/apply-grant-plan creates/removes 全 200（DATA 被转授校验 20040 拒绝=门禁生效实证）+ 浏览器全场景（入口预选/主体树/岗位切换/类型切换/授权弹窗/变更清单/保存/撤销/详情抽屉/DB 终态清零）+ 截图视觉核对 5/5；subject-tree.spec +3 用例，vitest 205/205，tsc 0 错。**Phase 3 九个联调任务全部收口**。
...
- 2026-09-02：**T-FE-020 收口（Phase 3 第四个联调任务 done，重建库后浏览器冒烟通过）**：条件/冲突规则两页 Gateway bootstrap 清单 +9 端点（condition create/update/remove + conflict-rule list/create/update/remove/detect + abstract-role/list——T-FE-016 登记的届时注册事项闭环；condition/list、type-definition/list、operation-permission/list 先在册；两个 detail 端点页面不消费不注册）；业务门禁零新增（CONFLICT_RULE 四档与 CONDITION 写三档 T-PERM-029/030 已预置固定图）；联调发现并修复前端路径缺陷 1 个（api/conflict-rule.ts 全 6 端点裸 /api/perm/** 未随 T-FE-041 切 Gateway 路径——Gateway 仅路由 /admin/**、/perm/** 必 404，统一修正 /perm/api/perm/conflict-rule/*）；mock/conflict-rule.ts 整删 + mock/permission-condition.ts 删文件保 _shared store（授权页 mock 仍消费，T-FE-018 届时处置；设计定案）；契约零漂移；PgIT 计数 45/44/92→54/53/101（scopeAll 47 不变）；description 清空语义登记已知差距（设计定案：恒传字符串空串写入，无恢复 NULL 通道，本任务不引入 descriptionClear）。终态见任务卡完成记录。
- 2026-09-02：**T-FE-017 收口（Phase 3 第三个联调任务 done，重建库后浏览器冒烟六场景全过）**：资源/操作定义页 Gateway bootstrap 清单 +8 端点（tree/operation list 先在册；operation detail 与 resource list 不消费不注册）、业务门禁 +4（RESOURCE/OPERATION CREATE+MANAGE scopeAll 死锁防护）、PgIT 计数 37/36/80/43→45/44/92/47（首轮 84 漏计 +8 路由各派生一条 API:ACCESS 实例授权，实测 92 订正）；联调修复 1 个后端缺陷（createResource 未设 maintainSource——flex insert 显式带列致 DDL DEFAULT 不生效、NOT NULL 500，创建链路打不通；修复 + ResourceOperationKeyPgIT DB 级回归锁）；mock/resource-operation.ts 路由段删除保数据导出（设计定案：仍被三 mock 引用，随各自页面联调退役）；契约零漂移（SDK ResourceUpdateReq id 轨漂移登记已知差距）；dev 密钥四变量换新值两服务同值重启。终态见任务卡完成记录。
- 2026-09-02：**T-FE-021 收口（Phase 3 第六个联调任务 done，重建库后浏览器冒烟通过）**：业务域配置页 Gateway bootstrap 清单 +9 端点（biz-domain 5 + domain-config 4）；业务门禁零新增（DOMAIN:VIEW 已在图 OPERATION_LOG 先例、SYSTEM_CONFIG:VIEW/MANAGE 已在图系统配置页门禁）；联调修复前端路径缺陷 1 类（api/biz-domain.ts + api/domain-config.ts 全 9 端点裸 /api/perm/** 未随 T-FE-041 切 Gateway 路径——Gateway 仅路由 /admin/**、/perm/** 必 404，T-FE-020 conflict-rule 同款，统一修正 /perm/api/perm/* 前缀）；mock 四文件整删（biz-domain/domain-config/两个共享注册表——仅 mock 间互消费，login.ts 只引 src perms.ts）；PgIT 计数 57/56/104→66/65/113（scopeAll 47 不变）；契约零漂移（9 组 DTO 逐一比对）。终态见任务卡完成记录。
