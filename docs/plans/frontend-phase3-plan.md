...
- 2026-09-04：**T-FE-037 双轨子代理评审收口（代码轨 P1×1+P3×6、文档轨 P2×1+P3×5，逐条代码级核实全属实；四项用户决策）**：P1 修复——preset 同主体判定改双字段比对 `isSamePresetSubject`（跨入口 id 碰撞防授权目标静默错乱，spec +4 用例、vitest 209/209）；P2 残留清扫 9 处「组织入口二期」现在时表述（types/perms/index.vue/perm.ts/role-manage.md）+ §13.2 决策 4 修订标 + user 页 mock 时代注释；用户决策四项落地——多标签直切不一致=登记已知边界、停用入口按钮/SSOT 归属=登记后续处理、ORG:VIEW 缺失占位=补冒烟（测试角色仅 ROLE:VIEW 投影实测，左栏占位+零网络请求，测试数据全清）。
