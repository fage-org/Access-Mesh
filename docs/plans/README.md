...
| [access-post-merge-plan.md](access-post-merge-plan.md) | 归并后续强化 | proposed | 承接归并主链后续任务 T-ACCESS-013（OAuth2 资源服务器）/014（✅ 操作日志覆盖）/015（菜单写链路收口），均已 done。准入前置以 CI 退出状态为准（原登记 40 项为当时口径；68 项为 2026-08-22 外部主机历史基线），由 T-ACCESS-017 最小 CI 落地、T-ACCESS-026 归档时关闭 |
| [product-vertical-slice-plan.md](product-vertical-slice-plan.md) | 产品垂直切片与试点加固 | proposed | 2026-08-22 风险基线全量核实立项、2026-08-23 评审修订：模型收敛（引擎显式资源 API → 统一主体 ID B-lite → 资源类型五组合并 → USER/ROLE 投影）→ 空库 bootstrap（固定租户 1 + 幂等 runner）→ 前端真实登录+导航收敛 → BASIC_ROLE E2E 垂直切片 + README 回写（里程碑 A）→ 试点加固 + example 接入（Gateway 主线）+ 证据收口（里程碑 B，硬门禁：全部 B 任务 depends_on T-ACCESS-021）。最小 CI 前置并入 T-ACCESS-017。18 项任务（T-ACCESS-016~026、T-PERM-042/043、T-ORG-001、T-ADMIN-022~024、T-GW-007、T-FE-041、T-API-001；T-FE-042 cancelled 并入 T-FE-041） |
