...
| [permission-grant-ux-refactor-plan.md](permission-grant-ux-refactor-plan.md) | 权限授予页授权弹窗与右栏变更重构 | active | 保留三栏和左栏角色树；中栏改资源权限概览+授权入口，授权弹窗批量授权，右栏改本次变更记录。任务 T-FE-025~028；§16.8 风险 R1~R11 全关，已转 active |
| [permission-grant-v2-plan.md](permission-grant-v2-plan.md) | 权限授予页 V2（方案A多条件分支模型） | active | 新增 /system/permission-grant-v2，与旧页面并存对比。直接操作矩阵+分支列表+fetchBaseline 原子恢复。任务 T-FE-029~035；门禁已关闭，可启动 T-FE-029；基于 state-model/interaction/error-flow 三份定案文档 |
