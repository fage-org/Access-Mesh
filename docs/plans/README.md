...
| [access-capability-fusion-plan.md](access-capability-fusion-plan.md) | access-service 能力包融合 | proposed | 2026-09-13 立项：两域融合——能力包终态（17 顶层包 = 12 能力包 + sync + engine + projection + bootstrap + infrastructure）+ 平行设施归零 + 概念单点收口（操作码 USER 细粒度化 / 字段消减 / system_config 单入口）；T-ACCESS-032~041（032 已 done）；目标设计 [../design/access-service-capability-structure.md](../design/access-service-capability-structure.md)（**adopted**——032 已完成 §8 归属清单十项裁决定稿），定案见 decision-registry 2026-09-13 两行 |
