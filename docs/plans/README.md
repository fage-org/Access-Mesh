...
| [permission-query-unification-plan.md](permission-query-unification-plan.md) | 权限查询统一引擎重构编排 | proposed | 2026-09-09 grill 定案（Q1-Q15+D1/D2）立项：T-PERM-057（引擎统一+判定面继承+评估拉平，含已取消 T-PERM-045 的范围）/T-API-003（check 族全量回传，推翻 T-API-002 check 族部分）/T-PERM-058（depend_on 单点闭合设计）/T-PERM-059（视图/排查删除重设计）；终态设计 `design/permission-center/query-engine-unification.md`（evolution，落地并入 implementation §3） |
