...
acceptance:
  ...
  - "操作码 fail-closed（2026-08-30 经决策）：create/update/batch-sync 收到解析不到的 operationCodes 拒绝 20005 OPERATION_NOT_FOUND——原实现静默丢弃（部分丢弃合并已知位、全 miss 落 0）会写出语义错误规则且无报错（例：requiredOperationCodes 拼错 → 落库 bits=0 = 什么都不要求，补全永不触发）；全空白码列表拒绝 20044"
  ...
  - "测试：DependencyAppServiceImplTest 重写扩充 30 项（原 5 项 autoGuard 用例保留：门禁五档+20019 先解析后门禁+PUT 全量覆盖 UpdateWrapper.getUpdates 显式 null 断言+20054 预查/DIVE 转译+20005/20044+FULL diff 三元组+N+1 times 锁定+DTO Bean Validation）；AccessBootstrapPgIT 授权计数 34→39/scopeAll 21→26"
...
4. **门禁/PUT/bootstrap 等其余八项为修法唯一**（029/030 先例 + 前端 Q3=B 既定契约 + schema 四值口径），未单独立项问询直接落地。

## 已知限制（登记）

- **batch-sync 条目资源未解析静默跳过**：同步清单漂移由对账兜底（与单条写入口 20004 fail-closed 不同语义）；操作码未解析则 fail-closed 20005（同单条口径）。
...
- 测试：DependencyAppServiceImplTest 30 项全绿（五组：AutoGrantGuard 5 / ReadEndpointsViewGate 4 / UpdateSemantics 8 / CreateValidation 4 / RemoveSemantics 2 / BatchSync 4 / DtoBeanValidation 3）；AccessBootstrapPgIT 39/26 计数锁定。
- 前端：api 类型+注释收口；mock 对齐（错误码同码、新字段、bits 字符串、remove data=null）；vue-tsc 干净 + vitest 216 全绿 + eslint 干净。
- 文档：api-contract §5.6 契约要点块 + §6.9 规则 + last_reviewed；core-flows §12 FULL diff 三元组句；resource-dependency.md §3/§4/§5 + last_reviewed；看板 031/T-FE-011 行；plan 进度行；AGENTS.md 阶段句。
