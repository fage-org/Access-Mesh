...
acceptance:
  ...
  - "操作生命周期联动（grok 外评存量升级定案「补联动」）：updateOperation 位有效变更（自定义类型）同事务迁移种子（软删旧位行+补种新位）；deleteOperations 同事务级联清理被删自定义类型操作的种子行（is_system 不联动）；两入口共持 RESOURCE_ENTITY 树写锁并锁内重读（update 锁内重绑 typeValue——同码重建交错下锁前值指向死号）"
...
- 2026-09-12 实施收口：
  ...
  - grok 外评处置（同日，用户显式触发；范围 025936e97..62c02f279）：P0/P1/P3=0；P2×1（createOperation 锁内未重绑 typeValue——「删类型→同码重建（typeValue 不复用）」交错下操作行+种子落已级联清理的死号、活类型缺操作死锁复发）核实成立已修（锁内 `resourceType = typeDef.getTypeValue()` 重绑 + 两类回归锁：锁内空 20021 不落库、锁前后值不一致写入落锁内值）；存量①（操作位变更/删除不维护种子→死锁复发/inert 死行）经用户拍板「补联动」落地（updateOperation 位变更迁移种子 + deleteOperations 级联清理 + 锁协议对齐 + 契约 §5.3 成文 + 单测五面）；存量②③（TypeForm 注释漂移、extension-guide 值域缺口）已修；专项七项全过、claude 处置复核成立（P2 撤回论证被独立确认）。
