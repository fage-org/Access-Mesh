...
- **bootstrap 图两处类型级化**（用户决策 2026-08-26，消除新服务接入鸡生蛋）：
...
| 验证项 | 命令 | 结果 |
| --- | --- | --- |
| example 单测 | `mvn -pl example-service test` | Tests run: 5, Failures: 0（DemoControllerTest 3：回显/30001/30002；ExampleServiceApplicationTest 2：瘦身后上下文启动+信封回显+全局异常 30001） |
| example E2E（三子进程 403→200） | `mvn -pl gateway test -Dtest=ExampleProtectedApiE2EIT` | Tests run: 6, Failures: 0（约 95s；④真实 403 → ⑥ 200+身份回显+30001） |
| access-service 单测轨 | `mvn -pl access-service test -DskipTestcontainers=true` | Tests run: 736, Failures: 0（含 EPP 断言修复后 AccessServiceApplicationTest 10/10） |
