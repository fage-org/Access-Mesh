...
8. **外部评审修正定案**：名称剪枝自**透视顶层**起（节点级过滤可能把透视节点与配置根断链，自配置根起剪会误删命中节点；关键词全不命中→空结果不返回裸根骨架）；`operationCode=CREATE` 前端接线（用户决策「本次顺带接前端」：创建用户表单挂载点树传 CREATE 且不携带 treeConfigId，组件加 operationCode prop；门禁后果——仅有 USER:CREATE 无 ORG:CREATE 的用户挂载点树 403 空态）；`checkAndThrow` 主体缺失仍走 SecurityException 403（既有全局语义，任务卡 P2-1 定稿「checkAndThrow 保持不动」，99999 语义仅限 hasTypeLevel）。
