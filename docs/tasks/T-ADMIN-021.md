...
doc_type: task
id: T-ADMIN-021
title: org-tree 扩展 includePositions（组织+岗位一体树，授权页主体树数据源）- **二期，首期不推进**
status: proposed  # 二期，首期不推进（首期只角色入口）
...
> 状态：proposed（**二期，首期不推进**——首期只角色入口，组织入口延后）
> 复杂度：🟡 中
...
> **重基线（T-ACCESS-012，2026-08-22）**：落点为 access-service admin 域单模块；门禁经本地 `PermQueryEngine`（`AdminPermissionValidator`，无 Feign 自调用），原 permission-center 不可达故障注入语义与 RoleProxyServiceImpl 参照物已失效；原 services/admin-service.md 引用已随该文档归档移除，契约以 admin-service-api-contract §4.2.1 为准。

## 实现要点

- 扩展 admin 域 `org-tree` 接口：请求体新增 `includePositions`（boolean，默认 false 兼容现有调用）。**请求/响应/兼容行为/权限裁剪以 `docs/design/services/admin-service-api-contract.md §4.2.1` 为准（P1-6）**：`includePositions=true` 时**忽略 `orgType`**（返回组织+岗位一体树，与 orgType=1/2 单类型语义互斥）；默认 false 时行为与现状完全一致（orgType 必填，`ORG_TYPE_REQUIRED` 保留，P1-2）。
