...
| [T-FE-046](T-FE-046.md) | 强制改密闭环（自助改密页+阻断）（✅ 2026-09-20 收口：登录写 userId/forceResetPwd 入 userKey+守卫 8 路径放行清单阻断 redirect /change-password+/change-password 页（8-32 位无旧密码）+改密成功清标记会话保留+登录 warning 分支退役；双轨评审 P2×2（/redirect 死项删+补锁、契约/后端注释滞后顺改）+P3×2 全处置；两项拍板（toast 维持现状、E2E 登记计划归档时跑）；红跑 6 红实证，见 registry 定案行） | [frontend-session-consistency](../plans/frontend-session-consistency-plan.md) | design/frontend/login.md | — | ✅ | ✓ |
