...
| T-FE-055 | 授予入口按钮按 MANAGE/VIEW 分文案（✅ 2026-09-20 收口：三入口文案二分「权限授予」/「查看权限」——grant-entry.ts 单源、门禁维持 ROLE:VIEW、canGrant 同批换绑授予页 SSOT 值等价；GrantMatrixPanel 授权按钮 view 态 v-if 隐藏→禁用态+tooltip 对齐 §10 表原文；三项拍板与维持现状处置见 registry 同日行；spec 模块缺失红起+两态锁；vitest 356/356+typecheck/lint/build 全绿；双轨评审 P0-P2=0、P3×5（重叠计 4+1）逐条亲核属实全直修、过度设计可裁剪项=0） | [frontend-session-consistency](../plans/frontend-session-consistency-plan.md) | design/frontend/permission-grant.md；design/frontend/role-manage.md | — | ✅ | ✓ |
