...
| [T-PERM-070](T-PERM-070.md) | 前置·公共服务认证模块（per-service 静态凭证：仲裁器/白名单/Gateway M2M/信任域 TLS） | —（T-PERM-035 实现序列前置卡） | docs/design/service-authentication.md | — | ⚙️ | ⏳ |
...
| [T-FE-045](T-FE-045.md) | 登出真注销（接 /auth/logout）（✅ 2026-09-19 收口：logout() 显式携 Authorization 头+http 请求白名单纳入防过期分支递归；logOut 服务端注销优先/本地清理无条件/串行防抖；回归锁×7 红跑实证；login.md 新增登出流程节；claude 外评 P2×1 维持现状+P3×1 登记 Q-016，见 registry 处置行） | [frontend-session-consistency](../plans/frontend-session-consistency-plan.md) | design/frontend/login.md | — | ✅ | ✓ |
| [T-FE-046](T-FE-046.md) | 强制改密闭环（自助改密页+阻断）（✅ 2026-09-20 收口：登录写 userId/forceResetPwd 入 userKey+守卫 8 路径放行清单阻断 redirect /change-password+/change-password 页（8-32 位无旧密码）+改密成功清标记会话保留+登录 warning 分支退役；双轨评审 P2×2（/redirect 死项删+补锁、契约/后端注释滞后顺改）+P3×2 全处置；两项拍板（toast 维持现状、E2E 登记计划归档时跑）；红跑 6 红实证。claude 外评处置完毕（2026-09-20 模型 deepseek-flash[1m]：P0-P2=0、P3×2 全采纳直修——纯空白密码 whitespace 规则+锁（后端 isBlank 随机替换锁死用户）+登录落点三处文案同步（MemberTab×2 退役口径+半成功「进入系统后」限定）、存量×6 处置（改密页无登出入口注记入卡）；vitest 325/325；定案与明细见 registry 同日处置行） | [frontend-session-consistency](../plans/frontend-session-consistency-plan.md) | design/frontend/login.md | — | ✅ | ✓ |
...
| [T-FE-048](T-FE-048.md) | 会话权限热刷新（403 触发+手动入口）（✅ 2026-09-20 收口：`refreshSessionCapability` 统一入口（权限串+侧栏原子更新，T-FE-056 挂接点预留）四调用方共用（initRouter/403 自动/顶栏手动/retryLoadDeps）；http 拦截器 403 分支 10s 窗口去重+失败静默+不重放+排除 user-menu 自身；顶栏「刷新权限」成功/失败双 message 口径；红跑 8 红/9 绿实证（三把验收锁全红）；双轨评审 P0-P2 代码轨=0/文档轨=P2×1（registry 补登记即本批完成）+P3 处置×5（三项拍板接受/登记 Q-016、重试按钮补 loading 采纳）；claude+codex 双通道外评处置完毕（2026-09-20：claude deepseek-flash[1m] P0-P2=0/P3×1、codex luna max P2×2，逐条亲核成立；两项拍板均登记不修——同会话并发覆盖 Q-016 两变体扩行、mix/horizontal 布局缺口修法定向抽公共组件暂不实施；零代码改动；定案与明细见 registry 同日处置行） | [frontend-session-consistency](../plans/frontend-session-consistency-plan.md) | design/frontend/login.md；design/frontend/permission-grant.md | — | ✅ | ✓ |
| T-FE-049 | 登录半成功语义修正（✅ 2026-09-19 收口：登录提示按菜单状态两态——半成功/零权限 warning 诚实告知（非阻断，`resolveLoginMessages` 唯一出口，T-FE-046 阻断分支扩展点）+ store `menuLoadFailed` 空侧栏两态事实来源 + 占位项/`/menu-retry` 着陆页两态（失败可重试 vs 无可用菜单引导联系管理员）；红跑 9 红实证；双轨评审处置（P2 registry 补行+P3×3+Q-020）+ claude 外评处置（P2×1 占位项未注册 name 删+补锁、P3×2 mock 注释/meta.title 中性化、存量图标 2 枚顺带注册+Q-021 登记）；vitest 303/303 全绿） | [frontend-session-consistency](../plans/frontend-session-consistency-plan.md) | design/frontend/login.md | — | ✅ | ✓ |
| [T-FE-050](T-FE-050.md) | findParentOrgName 递归 bug 修复（仅 user/index.vue）（✅ 2026-09-19 收口：未找到返 null 不剪兄弟子树，提取 utils/orgTree.ts；红跑 4 红 1 绿实证；Q-017 拍板留给 T-FE-051；双轨评审 P0×1+P3×3 处置（rule §2 对齐工具链/Q-018/051 锚点）；claude 外评 P0-P3=0+存量×2（Q-018 注记/Q-019），见 registry 定案行） | [frontend-session-consistency](../plans/frontend-session-consistency-plan.md) | — | — | ✅ | — |
| [T-FE-051](T-FE-051.md) | 列表加载统一错误处理+请求代际（composable 化）（✅ 2026-09-19 收口：useListLoad/usePagedList 两层 composable + toErrorMessage 统一错误文案（非 2xx 优先后端 body message，service-interface 局部副本退役）；16 加载面顺扫/收敛（user 页试点+八页代际+reqSeq/configReqSeq/treeRequestSeq 三先例收敛，matrixToken 矩阵面不收敛）；Q-017 就地化拍板收敛+启停移入 hook 透后端文案；红跑 user 7红/config 2红/pcl 1红实证；失败语义统一保留旧数据（pcl/resource-dependency 两页+biz-domain 子表刷新路径变更）；vitest 279/279 全绿；claude 外评 P0-P2=0+P3×3 全采纳处置，见 registry 处置行） | [frontend-session-consistency](../plans/frontend-session-consistency-plan.md) | — | — | ✅ | — |
| T-FE-052 | 跨层级拖拽+角色禁用加二次确认 | [frontend-session-consistency](../plans/frontend-session-consistency-plan.md) | design/frontend/role-manage.md | — | ✅ | ✓ |
