...
| [T-PERM-079](T-PERM-079.md) | 自动授权批次风格与可读性清理（070/074/071 触达面）— ✅ 2026-09-21 收口：七项验收全落地（参数组 record/字面量常量化/服务判据单源/分批样板收敛/var 定调=登记允许/次要项/拆档）+两项拍板 registry 同日登记（skipHeavyIT 新开关并入日常形态命令）+收口全量三段全绿（access-service 305+1308 含 heavy/e2e 15） | —（评审转出单卡） | —（工程改进；registry 2026-09-21 风格定案行） | — | ✅ | ✓ |
