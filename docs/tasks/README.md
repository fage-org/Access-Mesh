...
| ID | 标题 | 状态 | 直接依赖 |
|---|---|---|---|
| [T-PERM-080](T-PERM-080.md) | （R2-T01）全仓调用与语义清点 | ✅（2026-09-25 收口：清点册=计划附录 A〔零代码变更〕+设计 §6.5 增补与行内勘误；双轨评审 P1×2+P2×5 全核实直修；三项拍板见 registry 同日行） | — |
| [T-PERM-081](T-PERM-081.md) | （R2-T02）PQ-01/06 反例与正常语义基线 | ✅（2026-09-25 收口：三件资产〔共享夹具+互斥反例 PgIT+四族基线 PgIT〕容器轨绿跑全过+红跑取证入卡；断言=锁当前行为〔T-PERM-083/095 修复时翻转〕；三项拍板与索引序发现见 registry 同日行） | T-PERM-080 |
...
| [T-PERM-071](../archive/2026-09-24/tasks/T-PERM-071.md) | 独立依赖声明与可选 SDK 协调（✅ 2026-09-21 收口：全量含 E2E 1842 项 0 失败；双轨+四任务内外评审处置完毕〔claude P3×3 全处置、风格转出 T-PERM-079、拍板与豁免登记见 registry 同日三行〕；迁移未在任何部署库执行） | —（自动授权实施序列） | [简化设计](../design/dependency-auto-grant.md) | T-PERM-070, T-PERM-078, T-PERM-074 | ✅ | ✓ |
...
| [T-PERM-079](../archive/2026-09-21/tasks/T-PERM-079.md) | 自动授权批次风格与可读性清理（070/074/071 触达面）— ✅ 2026-09-21 收口：七项验收全落地（参数组 record/字面量常量化/服务判据单源/分批样板收敛/var 定调=登记允许/次要项/拆档）+两项拍板 registry 同日登记（skipHeavyIT 新开关并入日常形态命令）+收口全量三段全绿（access-service 305+1308 含 heavy/e2e 15）+claude 外评 P3×2 直修；2026-09-21 单卡归档 | —（评审转出单卡） | —（工程改进；registry 2026-09-21 风格定案行） | — | ✅ | ✓ |
