...
| T-FE-049 | 登录半成功语义修正（✅ 2026-09-19 收口：登录提示按菜单状态两态——半成功/零权限 warning 诚实告知（非阻断，`resolveLoginMessages` 唯一出口，T-FE-046 阻断分支扩展点）+ store `menuLoadFailed` 空侧栏两态事实来源 + 占位项/`/menu-retry` 着陆页两态（失败可重试 vs 无可用菜单引导联系管理员）；红跑 9 红实证；vitest 302/302 全绿） | [frontend-session-consistency](../plans/frontend-session-consistency-plan.md) | design/frontend/login.md | — | ✅ | ✓ |
