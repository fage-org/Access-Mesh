...
| [T-FE-048](T-FE-048.md) | 会话权限热刷新（403 触发+手动入口）（✅ 2026-09-20 收口：`refreshSessionCapability` 统一入口（权限串+侧栏原子更新，T-FE-056 挂接点预留）四调用方共用（initRouter/403 自动/顶栏手动/retryLoadDeps）；http 拦截器 403 分支 10s 窗口去重+失败静默+不重放+排除 user-menu 自身；顶栏「刷新权限」成功/失败双 message 口径；红跑 8 红/9 绿实证（三把验收锁全红）；双轨评审 P0-P2 代码轨=0/文档轨=P2×1（registry 补登记即本批完成）+P3 处置×5（三项拍板接受/登记 Q-016、重试按钮补 loading 采纳）；定案与明细见 registry 同日处置行） | [frontend-session-consistency](../plans/frontend-session-consistency-plan.md) | design/frontend/login.md；design/frontend/permission-grant.md | — | ✅ | ✓ |
