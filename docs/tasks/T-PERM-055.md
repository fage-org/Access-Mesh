...
- 回归证据：`mvn test -pl access-service -DskipTestcontainers=true`（2026-09-11）1129 tests、0 failures、0 errors；定向 `DomainClassifyServiceImplTest` 13 tests、`RoleManageAppServiceImplTest` 21 tests 全绿。
...
- `PermissionQueryAppServiceImplTest` 单独运行 0 tests：先于本任务存在（HEAD stash 复现、类内 7 个 @Test 无 @Tag 过滤迹象），与本次改动无关；如需排查另行立项。
