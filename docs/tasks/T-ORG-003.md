...
- **claude 外评处置（astron-code-latest headless plan，2026-09-22；P0-P1=0、P2×1+P3×3、过度设计=0，专项清单五项逐条执行+双轨处置五落点复核全过）**：P2——`setPrimaryOrg` 是唯一未持 SYS_ORG 树锁的成员关系写入口（T-ORG-002 挂锁枚举遗漏 set-primary）：并发 deleteOrg 级联软删目标归属行后，设主事务的清旧主 UPDATE 因 delete_flag=0 滤掉已删行、置新主命中 0 行——接口 200 但默认树主归属静默丢失 → 属实直修（与 assign/remove 同形补锁，门禁/存在性留在锁前、锁先于 resolveDefaultTreeOrgIds 守卫首读；拒绝路径不触锁不触守卫读；`UserOrgAppServiceTest` InOrder 锁序两锁，摘锁调用复跑实证 1 失败/带锁 2 绿）；P3-1——Q-025 类推收敛不完整：`UserAppServiceImpl`/`UserWriteAppServiceImpl` 两处 `validateOrgInDefaultTree` 逐字副本（findDefaultConfigs→get(0)→descendants 展开）仍手抄 → 两处换绑 `resolveDefaultTreeOrgIds` 共享入口（空配置/根失联统一折算 ORG_NOT_IN_DEFAULT_TREE，与旧空配置分支等价）；validateUsersInDefaultTreeScope 两处头部 findDefaultConfigs 仅做空判定（不复制展开逻辑，换绑反而整树展开加重）与 OrgAppServiceImpl 取配置实体变体（需新增领域读入口）维持现状不换；P3-2——接口声明层 `UserAppService.memberCandidates` javadoc 与 `OperationCode.ASSIGN_POSITION_USER` 注释仍写旧门禁 `ORG:UPDATE` → 两处改与实现同文；P3-3——存在性先于判权使 10101 与 403 可区分（本端点由本卡引入、与 set-primary/remove 既有同序一致）→ 契约 §7.2 验收句补可区分明示（收编为契约显性事实，不收紧——统一信封会与既有先例分叉）。存量观察转发：PositionTab org/users 失败时已分配人数静默渲染 0——T-ORG-003 验收无碍，T-ACCESS-055 有限管理员组合验收装配时注意该步失真形态；OrgOperationCodeMapper 大小写口径分叉（isPositionOrg 忽略大小写 vs Map 精确匹配）当前写入方不可达，留观不立项。处置验证：单测轨道全量 1365/0 绿 + 受影响三 PgIT（DefaultTreeDirectoryGuard/MemberCandidatesGate/UserCreateWithOrg）10/0 绿。
