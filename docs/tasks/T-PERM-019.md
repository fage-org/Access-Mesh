...
doc_type: task
id: T-PERM-019
title: 工作单 D：防呆机制（type_value 自动分配、业务键封装、AppliesTo、SyncHandler 版本声明）
status: proposed
...
acceptance:
  ...
  - "SyncHandler 声明支持的 payloadVersion 范围，sync_metadata 版本校验路径明确"
design_writeback:
  required: true
  status: pending
...
| D4 | SyncHandler 版本声明 | 与 `sync_metadata` / payloadVersion 思路兼容；需明确 min/max version 与降级行为 | `NO_HARD_CONFLICT` |
