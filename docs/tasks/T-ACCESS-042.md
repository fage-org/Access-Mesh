...
- **修复后验证**：前端 typecheck 0 错 + vitest 15 文件全绿（含新 auth.spec）+ build 3.43MB；gateway 全模块 106 全绿；access 定向 36 全绿；dev 冒烟重做——captcha 200 → login 200（7200）→ user-menu 200（2 菜单/50 权限）→ user/page 200 → 运行时端点匿名 401（负向）；收口全量回归 `mvn test -T 1C`（含 E2E）见完成记录增量。
