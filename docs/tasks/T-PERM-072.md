...
沿 071 承接 078 的实施前结论；M2 已定保留推导事实与精确去重，真实引擎验证仍须完成，M4 未收敛不得实现猜测版本。scope_all/父继承不作种子，权限含义是可独立使用的目标权限。T-PERM-075 的主体有效角色结果需组合核对，但本物化器按角色事实计算，不把 ROLE_MUTEX 混入依赖图或将 075 设为硬前置。
