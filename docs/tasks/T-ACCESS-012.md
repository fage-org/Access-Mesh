...
| 7 | 依赖图 | 设计变更待核对 7 行逐项收口（旧 DDL/错误码/陈旧回填/会话边界/10+5+15/api-contract 回写/admin-permission-sync 取代）；T-ACCESS-013/014 迁入 access-post-merge-plan（plan 字段与看板行同步）；无循环、无 dangling（按看板唯一权威清单口径：全部 84 条 depends_on 与各计划 tasks 均存在于看板；简单任务按治理模型仅占看板行、不开独立卡文件） |
...
- sys_menu 写链路 DDL-实体漂移（真实 PostgreSQL 下菜单写操作失败）按用户决策移交 T-ACCESS-015 承接（2026-08-22 评审；原 T-ACCESS-006 登记由本任务收口，因属功能开发改挂新任务）。
