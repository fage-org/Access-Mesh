...
**门禁**: `ORG:UPDATE@id` 实例级. 若 `parentOrgId` 变化, 还需 `ORG:UPDATE@新parentOrgId`. 操作码按目标 `orgType` 解析（Q-033 校准）：岗位目标实际判定 `UPDATE_POSITION`.
...
**门禁**: `ORG:DELETE@id` 实例级. 操作码按目标 `orgType` 解析（Q-033 校准）：岗位目标实际判定 `DELETE_POSITION`.
