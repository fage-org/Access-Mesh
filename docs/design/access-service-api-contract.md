...
| `ResourceTypeCode.ROLE` | `ROLE` | 被管理的角色实例（原 ADMIN_ROLE 并入；管理轨唯一消费方 /role/list 已退役〔T-FE-058〕，ROLE 常量面由 §10.3 abstract-role 族承载） |
...
| `/api/access/user-org/assign` | `ORG` | 实例级批量 (orgIds) | `MANAGE_MEMBER` / `ASSIGN_POSITION_USER`（按各 orgType 分组解析，T-ORG-003 校准） | 关系级追加; 默认树关系受身份目录边界二次校验 |
| `/api/access/user-org/remove` | `ORG` | 实例级 (orgId) | `MANAGE_MEMBER` / `ASSIGN_POSITION_USER`（按 orgType 解析，T-ORG-003 校准） | 非默认树仅删关系并回收对应 user_role; 默认树移除改按 `USER:UPDATE@userId` 身份目录高危处理 |
| `/api/access/user-org/set-primary` | `ORG` | 实例级 (orgId) | `MANAGE_MEMBER` / `ASSIGN_POSITION_USER`（按 orgType 解析，T-ORG-003 校准） | 首期仅允许默认组织树主归属 |
...
**门禁**: 与 `user-org/assign` 同权的成员动作码（T-ORG-003，F005 修复）——先验证目标组织存在（`ORG_NOT_FOUND`），再按目标 orgType 经 `OrgOperationCodeMapper.resolveForUserOrg` 解析：普通组织 `ORG:MANAGE_MEMBER@targetOrgId`、岗位 `ORG:ASSIGN_POSITION_USER@targetOrgId`（实例级）。不再要求 `ORG:UPDATE`（成员动作权与组织结构编辑权解耦，v1.4 口径）。
...
**验收要点**:
- 候选集**严格**来自默认树中操作者具备 `ORG:VIEW` 的组织范围内的用户; 不暴露全租户用户.
...
**门禁**: `USER:DELETE` 实例级批量 (`checkBatchInstanceLevel(USER, ids, DELETE)`) + 默认树边界二次校验 (操作者必须在每个目标用户的默认树主归属子树下具备可见性——ORG:VIEW 裁剪口径, 同 §7.2 候选范围机制, T-ORG-003 校准).
...
**响应（P1-3 定稿，T-ADMIN-021 已落地）**: `R<ItemsResp<OrgResp>>`，`data.items[]`（复用 perm-common 泛型 `ItemsResp{ items: List<OrgResp> }`，与 `/api/access/user-role/view` 同款——T-ADMIN-021 用户决策，原定稿命名的 `OrgItemsResp` 类不再新建，线格式不变）；**唯一形状，不再返回裸数组**（历史直返 List 已废弃）
...
- 默认树根节点不允许删除（无子节点同样拒绝；旧登记名 CANNOT_DELETE_DEFAULT_ROOT 已随 T-ORG-002 实现为 `ORG_DEFAULT_ROOT_DELETE_FORBIDDEN` 11017）.
...
**门禁**: 对 `orgIds` 按目标 orgType 分组解析成员动作码（T-ORG-003 校准，`OrgOperationCodeMapper.resolveForUserOrg`）——普通组织 `ORG:MANAGE_MEMBER@orgId`、岗位 `ORG:ASSIGN_POSITION_USER@orgId`（`checkBatchInstanceLevel` 分组批量校验）。
...
**门禁**:
- 非默认树关系: 按目标 orgType 解析成员动作码（普通组织 `ORG:MANAGE_MEMBER@orgId`、岗位 `ORG:ASSIGN_POSITION_USER@orgId`，T-ORG-003 校准）
...
**当前差距**: 无——默认树/非默认树门禁分支已实现（非默认树关系按 orgType 解析成员动作码（`MANAGE_MEMBER`/`ASSIGN_POSITION_USER`，T-ORG-003 校准）、默认树关系 `USER:UPDATE@userId`，默认树关系归 0 时拒绝）.
...
**门禁**: 按目标 orgType 解析成员动作码（普通组织 `ORG:MANAGE_MEMBER@orgId`、岗位 `ORG:ASSIGN_POSITION_USER@orgId`，T-ORG-003 校准）+ 默认树边界校验.
...
**迁移落点**：功能角色候选消费方（用户详情面板「分配功能角色」选择器）迁 `POST /api/access/abstract-role/list`（§10.3：`roleTypeCodes=[BASIC_ROLE,GROUP_ROLE,PERSONAL]` + `keyword` + 分页）——门禁随端点对齐实例准入口径（类型级 `ROLE:VIEW` 全量，否则可见子集裁剪、零可见 403），取代 T-ACCESS-052 范围拍板中「`/role/list` 维持类型级门禁」半边（`resource-api-mapping/list` 半边维持原登记）；分配动作仍受 `user-role/assign` 门禁约束，候选可见≠可分配。存量库 bootstrap 固定图该 API 行移除后资源行/映射/授权惰性残留（订正语句见 rebuild-runbook 常见问题表）。
...
- `update` 补 `extraClear` 显式清空标志（T-FE-016 联调收口，对齐 §12 resource-entity 同款口径）：boolean 可选，`true`=清空 `extra` 为 null（JSON null 无法区分「未传」与「清空」）；其余字段 null=不更新维持。前端编辑表单按「原 extra 非空且表单清空」判定传 `true`（resource-operation 同构公式）。**T-API-004（U006 拍板）起 `extra` 与 `extraClear` 同传 400 拒绝**——取代旧「true 优先于 extra」的静默丢值口径（全端点冲突拒绝统一协议，§2.7 清空协议注记）。
...
有 target 时从目标反向遍历直接来源；无 target 时遍历全集为推导图与未被 desired 支持的 actual AUTO_DEP 节点的并集，参与推导的种子（命中依赖边的显式根，2026-09-21 用户定案 B——未参与推导的显式授权不进解释视图，防孤点淹没推导链；完整授权清单走授权页 list）及孤立 actual 节点共同作为展示起点，按逻辑键排序后广度优先输出至 maxDepth/maxNodes；target 模式下选定目标事实保底入选输出节点——截断只挤祖先，不把查询目标本身挤出输出（2026-09-21 贴回外评定案，设计 §11 同批）。孤立 actual 节点也计入总数与预算，desired=false 且没有来源边，不伪造支持关系；只返回两个端点都已输出的边，再按起点/终点逻辑键排序取前 maxEdges 条，超限省略边也必须置 truncated。totalNodeCount/totalEdgeCount 是选定目标子图（无 target 为推导图与孤立 actual 节点的并集）的完整数量，不是已输出数量。actualPermissionIds 仅列对应 AUTO_DEP 行；显式 MANUAL 来源另由 seedRefs 表示。空数组表示无实际自动行，不能用 null 表示读取失败。
...
>
> **job 族门禁补齐（T-ACCESS-054，2026-09-24，仍维持未成册口径）**：`detail`/`page`/`log/page` 三读端点补 `ADMIN_JOB:VIEW` 类型级门禁（此前零门禁——任意租户登录用户可翻任务配置与执行日志，安全收紧）；bootstrap 固定图补 ADMIN_JOB 最小运营三档类型级授权（VIEW/TRIGGER/ENABLE，不可转授）——T-PERM-073 拍板的「管理员按需手动触发或启用周期巡检」正规入口（此前固定图零 ADMIN_JOB 授权=新部署全 403，运营只能改库）；CREATE/UPDATE/DELETE 维持无种子（改 cron 走运维通道）。同批对齐写端点门禁序：`update`/`toggle` 改为先门禁后存在性检查（与 `trigger` 及 T-ADMIN-029 公告族同形态，双轨评审存量观察顺手修——无权限统一 403，不再泄露 JOB_NOT_FOUND/403 的 id 存在性差异）。存量已初始化库重启将因缺三行（无软删墓碑）报「管理角色授权缺失」fail-fast 拒启（T-ADMIN-029 加行方向先例：固定图新增条目不自动补权），处置=按 rebuild-runbook 重建库。
>
...
> **角色互斥守卫（T-PERM-064；候选口径 T-PERM-075 U002 扩展 2026-09-22）**：BIND 的改写后窗口未过期时（含未来 valid_from 窗口；已过期不触发），「未过期原始持有候选（含禁用持有与禁用组子树）∪ 批内已 apply ∪ 本目标（含禁用目标）」命中 ROLE_MUTEX 对 → 该 item 返回 `NON_RETRYABLE`（`reason=ROLE_MUTEX_CONFLICT`）零写库——走通道逐条错误信封而非管理面 20062 整批语义；改写后未过期即检查（旧「幂等改期不触发」随窗口重叠判定消解——纯幂等重放因持有侧无冲突天然通过）。**full-sync 同批同用户多 BIND 经请求级批内累积判定**（原始持有批内一次预载 + appliedThisBatch 补偿，grok 外评 P1 修复沿承）——同批互斥两端第二条即拒，不依赖缓存可见性。UNBIND 不适用。
...
| 码 | 枚举 | 说明 |
|----|------|------|
| 20065 | `SERVICE_CREDENTIAL_INVALID` | 凭证定位失败 / secret 错误 / 半头 / 白名单外路径（仲裁器对白名单外的完整凭证头统一落本码，T-ACCESS-053 实测确认）（认证层 403 body 携带） |
...
合计: 18 项接口 (13 🔧 + 5 ✅)。原 `/api/access/user-role/assign`、`/api/access/user-role/revoke` 两行已随 T-ADMIN-024 端点删除移除（前端 `assignRole`/`revokeRole` 已随 T-FE-015 联调 2026-08-31 切换至 `/api/access/user-role/assign|revoke` items[] 契约）；`/api/access/role/list` 行已随 T-FE-058 端点退役移除（前端 `getRoleList` 删除，用户详情面板角色候选迁 `role-manage.ts` 的 `/abstract-role/list`）；api-gap-analysis.md "已核对接口汇总" 2026-06-21 归档至 `docs/archive/2026-06-21/`（其时点 16 个 🔧 已由 admin-service 实现，当前实数 13）. 另：后端 `OrgTreeConfigController` 存在 `/api/access/org-tree-config/{page,create,update,detail,delete,set-default}` 六端点（前端仅消费 `page`，已随 T-FE-015 注册 Gateway），本契约未展开登记——待树配置管理 UI 立项时补 §8.x 契约段（登记项，T-FE-015 联调发现）.

## 附录 B. 已对齐接口汇总（管理面家族，✅ 5 项）
| # | 接口 | 来源 record | 备注 |
|---|------|-------------|------|
