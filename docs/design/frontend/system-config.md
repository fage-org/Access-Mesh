...
- 无 `SYSTEM_CONFIG:VIEW` → **菜单不可见、路由被门禁拦 403**（menus 派生门禁，T-FE-056；fail-open 兜底形态为后端 403）；该兜底形态下进入页面后 `loadTable` 调 `/list` 由后端 VIEW 校验拒绝（403），前端 `message` 报错。本页无 VIEW 级按钮（VIEW 只决定列表数据可见性，由后端兜底）。
...
4. ✅ **list 服务端过滤+分页**（§9 预期、§8 原漏登，收口补登）：`SystemConfigListReq` = `{keyword?, pageNum?, pageSize?}`（替换 EmptyReq），返回 `PageResp`（keyword LIKE configKey/description、ORDER BY config_key,id）；分页参数均不传 = 字典全量（上限 200；原先例 `/api/access/role/list` 已随 T-FE-058 退役——字典全量族先例锚统一为 `/system-config/list` 本端点，族内其余端点引用本锚）；本页 hook 已切服务端分页。
