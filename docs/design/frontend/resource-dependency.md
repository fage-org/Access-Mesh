...
> **待实施演进**：自动授权采用[简化设计](../dependency-auto-grant.md)，由 T-PERM-071～073 实现。本文下述 CRUD 为现役页面行为，维持到实现回写；目标为只读诊断与按需来源，不持久化完整推导路径。M3 已定依赖仅由所属服务通过 manifest 发布，071 同步关闭手工写 API、移除新增/编辑/删除与旧批量同步入口，073 完成诊断与解释展示；无同 owner 手工编辑或跨 owner override。其余未决接口由 T-PERM-078 收敛。
