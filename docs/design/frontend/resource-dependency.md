...
> **待实施演进**：自动授权采用[简化设计](../dependency-auto-grant.md)，由 T-PERM-071～073 实现。本文下述 CRUD 为现役页面行为，维持到实现回写；目标为只读诊断与按需来源，不持久化完整推导路径。M3 已定依赖仅由所属服务通过 manifest 发布，071 同步关闭手工写 API、移除新增/编辑/删除与旧批量同步入口，073 完成诊断与解释展示；无同 owner 手工编辑或跨 owner override。解释接口与一致视图见契约总册 §12.3.1，manifest 见 §19.10；当前页面尚未实现。
...
- 目标资源类型下拉 + 目标资源下拉（联动）
- 触发操作多选（按源资源类型过滤；空=任意操作触发）
- 要求操作多选（按目标资源类型过滤，必填非空）
- autoGrant 开关（**默认 false 且禁用**——自动授权尚未交付；按[简化设计](../dependency-auto-grant.md)由 T-PERM-078 细化、T-PERM-071～073 实施，字段随声明通道落地退役，不再放开此开关；退役前后端拒绝 true，返回 20048）
