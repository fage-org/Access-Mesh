...
> **待实施演进**：自动授权采用[简化设计](../dependency-auto-grant.md)，由 T-PERM-071～073 实现。本文现役页面行为维持到实现回写；目标为显式/自动分组、按需来源和诊断，无跨 owner override，不持久化完整推导路径。未决接口与管理写边界由 T-PERM-078 收敛。
