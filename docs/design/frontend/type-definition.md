...
| description | 可空 | 描述 |
| isSystem | — | 新建默认 false（租户自定义）；编辑只读展示 |
...
- **加载**：进入页面 `getTypeDefList({pageNum, pageSize, typeKey, keyword})` → hook `loadTable`。
- **typeKey 下拉过滤**：顶部下拉选 typeKey（或「全部」）→ `onSearch` 重置页码 + `loadTable`。后端 `list` 现缺 typeKey 参数（见 §8 🔧 第 2 条），返回租户全量；**前端 typeKey 下拉本地过滤**（mock 已实现按 typeKey 过滤，Phase 2 后端补参数后联调）。
- **keyword 搜索**：按 name / typeCode 模糊匹配。
- **分页**：`onPageChange` / `onPageSizeChange`。

### 4.2 新增

- 顶部「新增」按钮（门禁 `TYPE_DEFINITION:CREATE`）→ 表单弹窗。
- 表单：typeKey 下拉（默认取搜索下拉当前值）、typeCode 可填或留空、name、description、isSystem（默认 false）、sortOrder、extra。**无 typeValue**。
- 提交 → `createTypeDef`（不含 typeValue）→ mock 校验 typeKey+typeCode 唯一（违反返回 409）、自动分配 typeValue（max+1）+ typeCode（若留空按 `${typeKey}_${name}` 规则生成）→ 成功 `loadTable`。
...
- 操作列「删除」按钮（`v-if="canDelete && !isSystemPreset(row)"`，门禁 `TYPE_DEFINITION:MANAGE`）→ `ElMessageBox.confirm` 二次确认 → `removeTypeDefs([id])` → mock 软删，isSystem=true 跳过（返回提示）→ 成功 `loadTable`。
...
| 操作 | 接口 | 请求 | 响应 | 核对 |
|---|---|---|---|---|
| 列表 | `POST /api/perm/type-definition/list` | `{typeKey?,keyword?,pageNum,pageSize,sort?}` | `PaginatedResp<TypeDefResp>` | 🔧 见 §8 |
| 详情 | `POST /api/perm/type-definition/detail` | `{id}` (IdReq) | `TypeDefResp` | ✅ |
| 创建 | `POST /api/perm/type-definition/create` | `{typeKey,typeCode?,name,description?,isSystem?,sortOrder?,extra?}` | `TypeDefResp` | 🔧 见 §8 |
...
1. **`typeValue` 自动分配未实现（T-PERM-019 D1 漂移）**
   ...
   - 期望：服务端在 `tenant+typeKey` 内自动分配 `typeValue`（`max+1`，软删不复用），`TypeCreateReq` 移除 `typeValue` 字段。
   - 依据：schema 唯一索引 `uk_type_definition_value (tenant_id,type_key,type_value) WHERE delete_flag=0`（permission-center.sql）保证租户+typeKey 内唯一；`type_value` 是内部计算值，不应外部入参。
   - 前端可行性：✅ 本页表单已不收集 typeValue，mock 自行 `max+1` 分配。后端切换后前端无需改动。
   - 归属：T-PERM-023 🔧（收敛 T-PERM-019 D1）。

2. **`list` 缺 `typeKey` 过滤参数**
   - 现状：`TypeListReq` 仅 `domainCode`（且未生效），返回租户全量。
   - 期望：`TypeListReq` 增加 `typeKey` 过滤参数，按 typeKey 分组查询。
   - 前端可行性：✅ 本页 API 签名已预留 `typeKey`，前端 typeKey 下拉 + 本地过滤。后端补参数后联调。
   - 归属：T-PERM-023 🔧。
...
- detail/update/remove 满足前端需求，请求/响应结构与 mock 对齐。
  - **list/create 不在此列**：缺 typeKey 参数 / typeCode 入参 / typeValue 自动分配，见 §8 🔧 第 1-3 条（T-PERM-023）。
...
- typeKey 下拉过滤为前端本地过滤（后端 list 返回全量）；Phase 2 后端补 typeKey 参数后切换服务端过滤。
- typeValue 自动分配为 mock 层 `max+1`；Phase 2 后端实现 D1 allocator 后联调。
- 联调（T-FE-022）需后端先补 §8 三项 🔧。
