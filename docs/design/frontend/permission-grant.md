...
- `TYPE_DEFINITION:VIEW` 依赖仍由 T-FE-018 联调门禁处理：前端声明只读依赖或后端提供免权限候选来源，未落地时阻塞真实接口联调。
