...
15. **条件权限不可转授（🔧 T-PERM-041，2026-08-05 确认；仅主权限；后端已随 T-PERM-041 落地 2026-08-30——20041 预检最终态判定 + 20042 条件启用校验同日落地，主权限 DDL CHECK 兜底）**：**主权限** `conditionCode != null` 时 `canGrant` 必须为 `false`——creates 主权限 + updates 结果态统一校验，违反 -> **20041** `CONDITIONAL_PERMISSION_CANNOT_DELEGATE`；**子权限不适用**（子权限不承载条件/再授予 -> 20043，见决策 17）；schema 增加 `CHECK (condition_id IS NULL OR can_grant = false)`（契约见 api-contract §6.5.1，DDL 见 access-service.sql）；本页授权弹窗选择条件后自动关闭并清除 canGrant（§4）。
