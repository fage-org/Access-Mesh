...
权限查看态的矩阵工具使用禁用+tooltip；footer“保存全部”和 MatrixCell“+”仍隐藏，不为不可用动作增加渲染。入口文案与 capability 在热刷新瞬间可能短暂不同，接受纯提示差异，后端门禁始终生效；不能将此边界扩为实际写权限差异。[来源](../../archive/2026-09-26/decision-registry-before.md)（原第 132 行）。
