...
| 组织主体适配器              | 本页自建（P1-2，T-FE-036 内）+ 依赖 **T-ADMIN-021** | 左栏主体树（组织入口）：**调用 org-tree 扩展接口（includePositions=true，岗位作为所属组织子节点返回，不分页；T-ADMIN-021 已实现 2026-09-03——含岗位节点按调用者 ORG:VIEW_POSITION 后端裁剪、引擎故障 fail-closed 99999、响应 {items} 包装；请求/响应/兼容行为/裁剪见 admin-service-api-contract.md §4.2.1，P1-6）**；不复用 ReOrgTreePanel（其 loadTree 固定 orgType=1 且岗位需分页拉取，混合需扩展共享组件影响 user 页） |
