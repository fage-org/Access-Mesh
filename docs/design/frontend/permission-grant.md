...
| 组织主体适配器              | 本页自建（P1-2，T-FE-036 内）+ 依赖 **T-ADMIN-021** | 左栏主体树（组织入口）：**已实现（T-FE-037，2026-09-04）**——SubjectTreePanel 组织分支内联适配器 + `subject-tree.ts buildOrgSubjectTree` 纯函数；调用 org-tree `includePositions=true` + `status=1`（仅启用，用户决策 2026-09-04）、默认树子树（不传 treeConfigId；多树租户其他树不在本入口可选范围，已知边界）；ORG:VIEW 缺失左栏占位（§10 轨道 1）。请求/响应/兼容行为/裁剪见 admin-service-api-contract.md §4.2.1（P1-6）；不复用 ReOrgTreePanel（其 loadTree 固定 orgType=1 且岗位需分页拉取，混合需扩展共享组件影响 user 页） |
