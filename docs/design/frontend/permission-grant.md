...
> **演进（已随 T-PERM-073 落地，2026-09-21）**：自动授权采用[简化设计](../dependency-auto-grant.md)。依赖声明管理已定只读，由所属服务发布；角色显式授权仍在本页操作。已实现：底部保存条「预览影响」手动按钮（用户定案 2026-09-21，草稿非空可用、edit 态显示）→ 抽屉展示 removed/added/retained 三组影响 + 仅供参考横幅 + 截断/既有漂移独立提示 + M5 边界说明四要素（`utils/preview-impact.ts` 纯函数 + vitest）；保存完全独立、不依赖预览结果——预览失败显式「无法预览」，保存成功即响应真实结果（失败保留标红整体重试，不把旧预览冒充提交结果）。分组计数按 removedTotal/addedTotal/retainedTotal 完整数渲染，截断时附「展示前 N 条」注记（2026-09-21 claude 外评处置，防分组头把截断输出数误读为组规模）。预览接口、只读准备与读取门禁见契约总册 §11.4.1。AUTO_DEP 记录维持「由资源依赖自动补全」标注（来源解释入口集中依赖页，registry 2026-09-21 定案）。
...
| ---- | ------------------------------- | --------------------------------- | ------------------------------------------------------------- | -------------------------------------------------------------------- |
| 角色 | 角色管理页（2.2）「权限授予/查看权限」入口按钮（文案二分 T-FE-055） | `ROLE`（T-PERM-043 后仅 BASIC_ROLE） | `abstract-role/tree`（权限中心）                              | BASIC_ROLE 可编辑（GROUP_ROLE 节点不展示，写入口已删除） |
...
- 3.2 权限条件页：复用型条件 CRUD（管理页条件轨）。本页弹窗条件控件双轨（T-PERM-048，2026-09-11）：**引用**管理页条件（picker 过滤 source=MANAGED）或**内联定义**单条授权专用条件（ReConditionEditor 就地编辑，随 apply-grant-plan 同事务创建/回收——内联 1:1 属于授权记录，管理页查不到也不能管理；门禁随 ROLE:MANAGE 携带不另查条件写权限）。
...
- **页面 capability 与状态正交**：顶层 `capability: 'edit' | 'view'` **仅由门禁派生**（ROLE:VIEW -> view；ROLE:MANAGE -> edit；GROUP_ROLE 主体 -> view 一支随 T-PERM-043 主体树隐藏而不可达）——**实现口径（T-FE-055 外评处置，2026-09-20）**：hook 以 `canManage` 响应式派生（`computed`），权限热刷新（T-FE-048 refreshSessionCapability）后即时重算（升权即时可授权/降权即时只读），未选主体不再是 view 态；旧「主体加载成功时 `setCapability` 快照」退役（快照在热刷新/未选主体形态下与门禁真相失真——codex 外评 P2，快照字段与 action 已从 grant-store 删除）；**不再由 AUTO_DEP 派生**。AUTO_DEP 降为**记录级** `readonlyReason: 'AUTO_DEP' | null`：AUTO_DEP 记录禁编辑/删除 + 悬浮标注（**不禁用整个单元格**，并存 MANUAL 记录仍可编辑/添加），与页面状态机互不干扰（同一页 MANUAL 可编辑 + AUTO_DEP 记录只读共存）。**降权边界（T-FE-055 复评处置，2026-09-20 用户拍板登记不修）**：降权瞬间已打开的授权弹窗仍可确认进草稿（`handleDialogConfirm` 不做能力校验），footer 保存入口随降权隐藏形成「有草稿、无保存入口」暂态——fail-safe（后端 `applyGrantPlan` 有 `ROLE:MANAGE` 门禁兜底、草稿仅存本地无副作用，可放弃或待权限恢复后保存），且该态不可从正常入口进入（view 用户无法打开弹窗）。
...
| 主体树（组织入口）         | admin-service `org-tree`（`includePositions=true`，岗位为组织子节点，T-ADMIN-021）                    | admin-service，`access-service-api-contract.md §8.1`（契约总册） |
...
| 条件列表                   | `permission-condition/list`（T-PERM-048：本页 includeInline=true 拉取含内联条件——回显内联名称/摘要；选择器侧过滤 source=INLINE） | api-contract.md §5.6（T-PERM-029/048）           |
...
| 条件选择器                  | 本页私有（T-FE-036 内自建；T-PERM-048 双轨扩展）   | 条件值域三态：无条件 / 引用管理页条件（搜索 + 摘要 + 启用过滤，**过滤 source=INLINE**）/「内联条件…」入口（select-inline 切内联编辑态，展开 ReConditionEditor 就地编辑，随授权保存创建）                                                                                                                                                                                                                                                                                                                         |
| 角色选择器/树               | T-FE-001 池（2.2 复用）                             | 左栏主体树（角色入口）                                                                                                                                                                                                                                                                                                                                                                                 |
| 组织主体适配器              | 本页自建（P1-2，T-FE-036 内）+ 依赖 **T-ADMIN-021** | 左栏主体树（组织入口）：**已实现（T-FE-037，2026-09-04）**——SubjectTreePanel 组织分支内联适配器 + `subject-tree.ts buildOrgSubjectTree` 纯函数；调用 org-tree `includePositions=true` + `status=1`（仅启用，2026-09-04 定案）、默认树子树（不传 treeConfigId；多树租户其他树不在本入口可选范围，已知边界）；ORG:VIEW 缺失左栏占位（§10 轨道 1）。请求/响应/兼容行为/裁剪见契约总册 access-service-api-contract.md §8.1（P1-6）；不复用 ReOrgTreePanel（其 loadTree 固定 orgType=1 且岗位需分页拉取，混合需扩展共享组件影响 user 页） |
