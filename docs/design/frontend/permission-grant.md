...
> **待实施演进**：自动授权采用[简化设计](../dependency-auto-grant.md)，由 T-PERM-071～073 实现。本文现役页面行为维持到实现回写；目标为显式/自动分组、按需来源和诊断，不持久化完整推导路径。依赖声明管理已定只读，由所属服务发布；角色显式授权仍在本页操作。M5 已定自动授撤预览仅供参考，提示结果可能变化；保存按服务端最新事实重算，影响变化不要求再次预览确认，成功后刷新实际结果。预览/刷新失败明确提示，不能展示为零影响或沿用旧预览冒充提交结果。正式预览接口、只读准备与读取门禁见契约总册 §11.4.1/§12.3.1，由 073 实施。
