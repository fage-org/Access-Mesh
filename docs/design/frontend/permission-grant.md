...
> **演进（已随 T-PERM-073 落地，2026-09-21）**：自动授权采用[简化设计](../dependency-auto-grant.md)。依赖声明管理已定只读，由所属服务发布；角色显式授权仍在本页操作。已实现：底部保存条「预览影响」手动按钮（用户定案 2026-09-21，草稿非空可用、edit 态显示）→ 抽屉展示 removed/added/retained 三组影响 + 仅供参考横幅 + 截断/既有漂移独立提示 + M5 边界说明四要素（`utils/preview-impact.ts` 纯函数 + vitest）；保存完全独立、不依赖预览结果——预览失败显式「无法预览」，保存成功即响应真实结果（失败保留标红整体重试，不把旧预览冒充提交结果）。预览接口、只读准备与读取门禁见契约总册 §11.4.1。AUTO_DEP 记录维持「由资源依赖自动补全」标注（来源解释入口集中依赖页，registry 2026-09-21 定案）。
