...
status: adopted
domain: frontend
last_reviewed: 2026-09-14（T-ACCESS-041：域叙事措辞「permission 域共享」→「权限面共享」一处）；此前 2026-09-13   # T-ACCESS-034：操作码常量类名机械改挂 OperationCode；此前 2026-09-11 T-PERM-048 双轨制收口：§9 新增（管理页条件轨定位、list 缺省只回 MANAGED 零改动、门禁/守卫后端变化、错误透传）；此前 2026-09-02 T-FE-020 联调收口（§API 依赖补 Gateway 路径联调注记、§组件结构 mock 终态化）；2026-08-30 §8 全量收口 + 字段/API 表对齐（T-PERM-029 落地：detail/update/remove 切业务键 code、detail 查不到 20006、list 全量不分页设计定案、ConditionResp 补 updatedAt、remove 幂等静默跳过）+ 写门禁口径收窄为类型级（CONDITION 无实例投影，实例级原声称系 ID 空间错位废弃；实例投影与条件双轨制登记 T-PERM-048）；此前 2026-08-08 产品确认移除 CONDITION:VIEW 读取门禁
---
...
| 接口 | 方法 | 请求 | 响应 | 权限 |
|---|---|---|---|---|
| /api/access/permission-condition/list | POST | ConditionListReq{includeInline?}（T-PERM-048：缺省只回 MANAGED） | ItemsResp<ConditionResp> | 无读取门禁（产品确认，全租户开放） |
| /api/access/permission-condition/detail | POST | ConditionDetailReq{conditionCode} | ConditionResp | 无读取门禁；查不到 20006（T-PERM-029） |
| /api/access/permission-condition/create | POST | ConditionCreateReq | ConditionResp | CONDITION:CREATE |
| /api/access/permission-condition/update | POST | ConditionUpdateReq{code,...} | ConditionResp | CONDITION:UPDATE@{code}（T-PERM-048 升实例级；INLINE 行 20060） |
| /api/access/permission-condition/remove | POST | ConditionRemoveReq{codes} | Void | CONDITION:DELETE@{code} 实例级全有或全无（T-PERM-048）+ 引用守卫 20059 |
...
> 与 RESOURCE/OPERATION 的 CREATE+MANAGE 两档不同，CONDITION 后端用独立的 CREATE/UPDATE/DELETE 三档（ConditionAppServiceImpl 对齐 `OperationCode.UPDATE`/`DELETE`，非 MANAGE）。
