...
| 权限查询统一引擎（演进终态） | [permission-center/query-engine-unification.md](permission-center/query-engine-unification.md)（`status: superseded`，2026-09-10 随 permission-query-unification 计划归档转正——四任务全 done，实现以 [implementation.md](permission-center/implementation.md) §3 为唯一权威，本文件仅存定案过程追溯） |
...
适用范围：根 `README.md` 特性表、`architecture.md` 能力/模块表、`permission-center/*` 与 `services/*` 的能力概述章节。各文档标注用词可与本表等价标注互换，但必须能唯一归入一档。
