...
- **等待取舍**：不增加 `tryLock` / `lock_timeout` 等待保险丝，以免误杀排在 full-sync 后的合法长等待；与 §17.2 不配置 statement_timeout 同向。故障恢复依靠重启持锁实例或 watchdog 停止后的 lease TTL 释放。锁模型或 full-sync 执行边界变化时复核。[来源](../archive/2026-09-26/decision-registry-before.md)（原第 41 行）。
