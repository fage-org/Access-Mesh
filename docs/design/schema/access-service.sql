...
COMMENT ON COLUMN resource_entity.owner_service_code IS '资源维护方服务编码；仅用于 service-config/sync、资源依赖等既有维护来源标记。新 resource-entity/sync/full-sync：本地管理投影（sourceService=admin-service 的 ADMIN_USER/ADMIN_ORG/MENU 等）显式写入 access-service；外部业务服务同步保持 NULL，其 ownership 以 sync_metadata 为准';
