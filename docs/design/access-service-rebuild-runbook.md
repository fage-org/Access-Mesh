...
| 升级到 T-ACCESS-054（2026-09-24）后：启用 bootstrap 的存量库**重启即 fail-fast**（`bootstrap 固定图冲突…管理角色授权缺失（缺行无软删墓碑=非管理端撤销，fail-fast）: 26#bits=2@ALL; 26#bits=16@ALL; 26#bits=64@ALL`）——与 T-ADMIN-029 同为加行方向（businessGrants 新增 ADMIN_JOB 三档 VIEW/TRIGGER/ENABLE 类型级授权，不在四个免除自愈通道内，新增条目缺行且无墓碑按残缺拒启，预期行为非缺陷；双轨评审 P1 订正初稿「自动补种」误冐） | `BootstrapGraphDefinition.businessGrants` 扩 ADMIN_JOB 三行（scopeAll、canGrant=false）——ADMIN_JOB 身份键历史上从未被授予，无墓碑可匹配 | 处置=**重建库**（本 runbook 主流程；开发期唯一实际路径，同 T-ADMIN-029 行口径）。空库新建直接获得三档；任务配置与执行日志无存量需订正 |
