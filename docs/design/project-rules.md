...
存量 `normalizeCode` 及同款裸 `toUpperCase/toLowerCase` 暂不做 Locale 专项整改；这是已接受的存量边界，原决定未记录理由。操作码与资源类型码的入站 raw 严格化以契约总册 §2.5 和引擎实现 §8.2 为准，不恢复授权侧大写归一。新增路径或大小写契约变化时重新核对适用范围。[来源](../archive/2026-09-26/decision-registry-before.md)（原第 30、104 行）。
...
**主键线格式边界**：BIGSERIAL 主键继续使用 JSON number；上面的十进制字符串策略用于位值及其他大数列，不把现有主键响应一并改为 string。主键分配策略或容量前提变化时另行核实，不把这项现行取舍表述为 JavaScript 可精确表示任意 bigint。[来源](../archive/2026-09-26/decision-registry-before.md)（原第 32 行）。
...
跨能力事实供给入口须保留调用方的事务新鲜度。T-ACCESS-043~046 收敛形成的实体命名供给服务采用本能力 Mapper、无缓存、随调用方事务，不机械换绑到带缓存的 loadRules 等入口；沿既有 SubjectDomainService/TypeResolutionService 宿主复用，Bean 依赖保持无环。此约束针对这些事实供给通道，不禁止其他 DomainService 按已声明边界使用缓存。具体归属见[能力结构设计](access-service-capability-structure.md)。
...
权限码采用“资源:操作”，资源部分使用名词性实体，不能用操作描述充当资源名。设计新门禁时先核对能否复用目标实例权限检查，再决定是否新增权限码。[来源](../archive/2026-09-26/decision-registry-before.md)（原第 34 行）。
...
对外可见的安全收紧或正确性收紧须补 `CHANGELOG.md` 的 `[Unreleased]` 对应条目；纯内部文档归位不虚构产品行为变更。[来源](../archive/2026-09-26/decision-registry-before.md)（原第 165、167 行）。
...
| 文档 | 职责 | 禁止 |
|---|---|---|
| `access-service-api-contract.md` | 字段/错误码/请求响应结构（契约总册，唯一详细来源） | - |
| 任务卡 `T-*.md` | 该任务详细范围/验收/背景/当前口径（唯一详细来源） | 评审轮次日记、「用户决策」过程清单 |
| `engine/implementation.md`/`engine/core-flows.md` | 实现设计/核心流程（引用契约总册，不重复字段定义） | 重复字段表 |
| `phase*-plan.md`/`tasks/README.md` | 任务清单（标题/状态/直接依赖/链接）+ 计数器 + 依赖图 | 复制"七项/八项/第几轮/完整字段清单/🔧 详细范围" |
| `decision-registry.md` | 定案导航：使用说明、稳定主题路由、历史入口 | 决定正文、逐案摘要、任务状态与评审处置流水 |
| `pending-problems.md` | 待解决问题登记（现象/证据/影响/状态/关联任务；暂不足以立任务的问题唯一载体，可执行即转任务、收敛后压缩为索引行；机制见 `design-plan-task-lifecycle` skill §2.5） | 复制定案正文（结论归所属权威规范）、转出后维护方案细节（唯一详细来源是任务卡） |
...
- 讨论过程留在 Git 历史或归档材料中；用户定案当轮归所属权威载体，正文表达当前结论与必要来源，按主题组织，修订保留独立有效语义及历史证据。写入与消费依下节统一协议。
...
<a id="decision-governance"></a>
### 定案归位与按范围消费

用户定案当轮落仓：先确定适用范围和权威落点，更新结论，再补来源与必要引用。不再向总表逐条追加；长期结论写所属设计/契约/Rules/Skills，已有章节直接合并。普通规则不强制套模板、不硬限字数，但不能删掉前提或边界。治理规则以本节为唯一权威，定案入口只负责定位。[采纳依据与迁移证据](../archive/2026-09-26/README.md)（T-ACCESS-064）。

| 内容 | 当前载体与边界 |
|---|---|
| 当前规则 | 所属规范表达现在采用什么、适用范围；易重复争论的取舍补必要理由及来源，不编造原记录未载明的动机 |
| 有效例外 | 紧邻被限定的规则，写明具体范围、接受影响、谁于何时确认及可追溯依据、事件复核触发；跨主题只有一个正文，其他主题链接 |
| 任务级安排 | 任务卡的范围/验收/当前口径，跨任务安排相互引用，不升格为全仓规则 |
| 修复与验证事实 | 代码/测试、任务完成证据或提交历史；“已修”不构成永久豁免 |
| 历史决定 | Git 版本或 docs/archive 原文，仅在解释、核对和追溯时读取 |

永久选择不采用某机制通常是当前规则；暂时不处理才可能是有效例外。问题尚未决定处理方式时仍在 pending-problems，不得自行写成用户接受风险。已有任务/计划载体不重复建问题；问题拍板后将结论归所属权威位置，问题条目只更新状态与引用。

**修订与生命周期**：以独立语义为单位更新当前正文。部分取代只移除失效部分，保留仍有效边界；不能按表行、任务归档或日期新旧推断有效性。实质替代注明范围及明确采纳依据，旧文本留历史证据。`adopted` 表示已采纳目标，不证明代码交付；存在迁移差异时同时说明目标、当前实现和被允许的过渡范围。代码存在也不等于行为已获授权。

任务收口核实长期结论已归位、任务安排已兑现、相关例外是否仍适用及引用可解析。关联任务 done 只能触发复核，须验证承诺的替代行为才解除例外；不要求所有例外设置日历到期时间。归档只迁历史载体，不撤销仍有效规范。重要跨文件引用使用稳定主题锚点；移动/更名时修复当前引用，历史证明可固定到版本。唯一证据提交须持续可取得，不仅存在于临时本地分支；历史改写前保存必要原文或更新来源。

**开始工作时**：从[定案入口](decision-registry.md)定位，取任务 `design_refs`、修改对象及调用关系、任务业务概念和跨领域约束的并集，读取相关章节与有效例外。不能只按文件目录判断主题，也不得默认整读历史登记、整本契约或全部设计。AGENTS 只保留跨任务硬约束与定位指针；新增工具注入面只做指向 AGENTS 的薄入口，不复制浓缩规范。任务进展、临时状态和待拍板议题以仓库为载体，不写入长期记忆攒议程。

**形成候选发现后**：报告前核对相关当前规范。恢复/删除既有机制、改变接口行为、扩大范围、重新处理已知风险或多来源冲突，必须核对原决定与替代关系。检索结合任务号、业务概念、类/方法及新旧名称，覆盖相关 docs、`.claude`、`.agents`；`rg --hidden` 限定指令目录，历史按入口所列快照或 `git log/show` 追溯，不把文件搜索零命中当作没有决定。命中后读足上下文。

| 候选发现 | 处置 |
|---|---|
| 相同范围和前提下重提已否决方案 | 引用当前规则，不重复要求用户选择 |
| 与有效接受边界完全相同且无新影响 | 标为已知边界，不重新包装成缺陷 |
| 实现违背当前定案 | 报告实现与定案冲突 |
| 旧问题已修但当前再次出现 | 按当前证据核实回归，不凭修复历史撤回 |
| 例外范围外问题、前提变化或影响扩大 | 单独评估并说明变化，不扩大旧豁免 |
| 依据不足或来源冲突 | 指出具体未核实项；不能直接认定缺陷或撤回 |

历史有记录只是线索，不自动构成撤回依据。冲突须核对采纳依据、适用阶段、局部例外和实现差异；不得规定登记表/代码/最新文件无条件优先。未消解的真实冲突保留证据并报告，不自行拼接为新决定。

双轨本地评审与外评使用相同对象版本、任务范围、相关规范位置及有效边界；各轨可独立扩展读取，不能被主代理摘录限制。报告列出支撑结论的依据；撤回争议发现须说明匹配决定及范围，依据留在本次评审结果，不再追加全仓禁报项。不再以“已读登记表全文”作为完整性证明。本地与外评仍分别按对应 Skill 触发。

<a id="decision-migration-gate"></a>
### 治理迁移的切换与回退

改变存储或消费协议时，先固定实施基线并保存原文，按独立语义建立一次性对照证据；核对现行册、历史册、已推翻内容和压缩前独有边界。所有有效语义有当前落点、可能影响行为的未决项清零后，入口、规则、技能双副本和引用才可同批切换；归位完成前不提前停止旧消费方式。

**切换前增量门槛**：记录初始基线与最终核对版本，对比期间源记录及受影响权威载体的新增、修改、取代项，逐项补入归位结果和迁移证据。未核对增量必须为零；核对后相关源再次变化须补核，不能仅凭最初快照完成就重写入口。纳入本次修改的工作树内容须有清单/哈希或最终提交可核对。无须为此建设常驻索引或 CI 服务。

先做语义、链接和场景验收：重复决定能定位、真实回归不误撤、局部例外不扩围、部分取代可直接读到当前组合、目标与实现有区分、任务归档后引用可追溯。丢失有效决定或误撤回真实问题均阻断切换。纯文档治理使用文本场景和文档检查，不用全仓业务测试替代语义验收。

回退须整体恢复一致的存储与消费方式，并将切换后新增/修订决定补回旧消费方式可找到的载体；不能只恢复某个 Skill 或覆盖成切换前快照而丢失新决定。原始快照正文冻结，历史协议由归档批次声明不再约束当前工作流。一次性证据随任务归档，不变成永久登记表；入口约 4KB 仅为职责检查参考，超出先检查是否混入逐案正文，不能为压体量截掉必要内容。
...
改 API 契约 + 任务卡（唯一详细来源）-> phase plan/README 只同步标题/状态/依赖/链接 -> 执行关键词扫描 -> 历史决定保留版本或归档（不新增独立 ADR 体系）。
