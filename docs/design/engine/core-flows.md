...
目标：为角色配置资源和操作权限，这是最核心的权限事实写入链路。**授权页面写链路收敛为唯一写入口 `apply-grant-plan`（2026-08-02 单入口收敛，2026-08-27 端点退役收口）**——所有页面写操作（新增/编辑/删除主权限与子权限、跨键替换）统一在一次 plan 中表达；`save/revoke/children/add-child/remove-child` **已从 Controller 删除（无映射 404，无存量调用方）**，`update-child/children-save/rebuild` 从未实现。
...
> **已规划、未交付**：采用[简化设计](../dependency-auto-grant.md)。070 服务认证与 078 协议校准已完成；071 编译/发布组件已落地、旧管理写入口与 autoGrant 已退役；072 角色物化已落地（AUTO_DEP 行经六入口同事务 diff 落库，运行时仍消费普通授权行、不遍历依赖图）。解释由 073 实施。
