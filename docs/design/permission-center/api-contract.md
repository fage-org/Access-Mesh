...
- `sync`/`full-sync` 的 parent 同款环路判定（评审收口补齐）：parent 为目标角色自身或其子孙时拒绝（单条 nonRetryable、批量该项 failed + `ROLE_PARENT_INVALID`）；判环先于版本推进（拒绝不消耗同步版本，上游修正后同版本重试不被判 STALE），full-sync 按「现有关系 + 本批次待更新边」组成的最终图统一判定（含同批次多边共同成环与批内/库内混合成环），防外部同步错误数据旁路 move 防护。
