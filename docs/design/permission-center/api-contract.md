...
- **写侧落地（T-PERM-034 收口，2026-08-30——T-PERM-033 登记的读侧依赖解锁）**：`apply-grant-plan` 按本节聚合形状写一条 `ROLE_PERMISSION_CHANGE` 日志——`items[]{changeType, permission(6 字段业务键), role 摘要}` 由 prevalidate 期业务键快照装配（creates 取请求键、updates/removes 反查资源实体+类型+按位操作，removes 行软删后不可回查故预检期快照；removes 的悬挂引用降级为 null 键字段，删除不被死引用阻塞），端到端场景（成功恰好一条聚合日志+缓存失效；失败全回滚不触发）由容器特征测试锁定。
