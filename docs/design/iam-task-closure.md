...
T-PERM-078 继续收敛[设计 M1～M5](dependency-auto-grant.md#implementation-decisions)，不再重选已采纳方向；M3 管理只读边界、M1 的 FULL 代次拒旧/完整空资源清单/资源停用不影响传播、M2 保留各操作条件事实并精确去重、M5 参考性预览与按最新事实保存已定。剩余发布协议、真实引擎验证、存量迁移、编译并发、解释/预览读取与正式接口继续核实。细节未定不得把卡标 done。
