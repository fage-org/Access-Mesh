...
**现象与证据**：T-ACCESS-039 将 catalog code `admin:org-visibility` 改名后，新旧实例的 `evictAll` 各扫自身 catalog code 命名空间互不可删（RedissonBucketStore 按 `{tenantId}:{catalogCode}:*` 精确扫描实证）；Nacos 旧键 TTL 覆盖同批被静默忽略。
