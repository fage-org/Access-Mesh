...
- **关联**：T-FE-045、T-FE-054（token 过期提示相邻面，可并入该卡触达）、T-FE-048（2026-09-20 双轨评审代码轨 P3-2：403/手动刷新通道扩大 `refreshUserMenu` 无代际守卫的同族竞态面——旧刷新响应晚到会把旧会话权限串写入新登录侧栏，用户拍板随本 Q 一并收口，明细见任务卡遗留节）
