...
复杂任务正文只允许这些标题（当前口径 = 已采纳规则的终态，禁止用轮次定位；终态记录节可选，只写终态事实与证据）：
...
## 验收对照
## 非目标 / 遗留
## 完成记录（可选；回归证据必须内联命令+日期+测试计数，或指向具体 commit 哈希，禁止指向不存在的记录区）
```

禁止「评审修复记录」「第 N 轮」「用户决策清单」等过程标题与过程流水；「完成记录」「执行记录」等**终态记录标题**允许（2026-08-28 收敛：与全仓既有任务卡实践及 project-rules 权威词表对齐——禁令对象是过程叙事，不是终态事实）。拍板后的规则回写 `design_refs` 指向的设计；任务卡只写终态或链到设计。轮次词扫描、任务卡与设计同等适用、已采纳「用户决策」写成当前口径：见 `docs/design/project-rules.md` §文档治理（本技能不复制禁令与词表）。
...
- **登记**：YYYY-MM-DD
- **来源**：<发现场景与终态出处锚点：任务卡遗留项 / 设计文档已知问题句 / 当前规则或历史决定锚点>
...
**设想方向（可选，未定案）**：<候选思路；一旦拍板归所属权威规范并建立引用，不在本文件展开>
```

**登记规则**：

- 触发场景：任务收口/评审中发现但不在当前任务范围（含用户拍板「记录问题、本次不解决」）；任务卡「非目标 / 遗留」延期项；设计讨论发现缺陷/风险但方案未定。
- **已有任务/计划载体的事项不登记**（2026-09-13 定案；含 done/cancelled 卡定案中「另立任务 / 新形态另立任务」的安排——看板、任务与所属规范已是其载体）；误登记的按重复收敛（closed 移入索引）。
- 必含**现象 + 证据锚点**与**影响**；未核实的问题不登记（先核实再登记）。
- **禁止**复制定案正文（定案正文归所属权威规范或任务，`decision-registry.md` 仅导航）；问题转出后禁止在本文件维护方案细节（唯一详细来源是任务卡）。
...
- `→ closed`：不解决/重复/失效须注明出处（所属规范或可追溯用户决定），同样移入「已收敛」索引表。
...
1. 核对 `acceptance` 全勾；核实长期结论已归位、任务安排已兑现、相关有效例外仍否适用以及来源/引用可解析（关联任务 done 只触发复核，不自动解除例外）；
2. 回归证据内联可追溯：命令 + 日期 + 测试计数（如 `mvn test` N 项 0 失败）或具体 commit 哈希；禁止指向不存在的记录区（如「见看板提交记录」——看板无提交记录区）；
3. 对每个 `design_refs`：打开设计文件，确认章节已反映本任务实现；未反映则**先更新设计**；
4. 按 `project-rules` §文档治理对任务卡与 `design_refs` 跑轮次词扫描；命中则先改写成当前口径，不得标 `done`；
5. 设 `design_writeback.status: done`，bump 设计 `last_reviewed`；
6. 若设计因此新增契约，可能需 `draft → adopted` 转换（§3.1）；
7. 设任务 `status: done`；
8. 检查所属 plan 是否满足 `completed` 条件（§3.2）。
...
3. 用户「不解决」拍板当轮归所属规范的接受边界并补来源，条目改 `closed` 并注明出处。
...
- 定案归位（含“不解决”拍板）→ [project-rules 文档治理](../../../docs/design/project-rules.md#decision-governance)；[定案入口](../../../docs/design/decision-registry.md)只定位主题。当前规则就近保存，问题清单只记问题与收敛状态，历史按需追溯。
