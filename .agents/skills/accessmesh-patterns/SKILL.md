...
- 禁止裸拼业务键：后端业务键构造/解析唯一入口 `perm-common` 的 `BusinessKeyUtil`（类型解析缓存键、操作位/操作编码键、资源三段键、转授五段键、relationKey 解析等 19 个方法族；格式由 BusinessKeyUtilParityTest golden 锁）；sync API 契约键（percent-encoded）走 access-service `SyncKeyCodecUtil`，缓存存储信封/错误文案拼接出界。
