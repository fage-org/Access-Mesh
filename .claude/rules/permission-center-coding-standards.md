...
| ------------- | ---------------------------------------------- |
| `ALL`         | 不过滤，查看全部                               |
| `GLOBAL_PLUS` | 全局域 + 指定域（指定域类型 + 全局域实际范围） |
| `DOMAIN_ONLY` | 仅指定域声明的类型                             |

### 全局域

- 每个租户有且仅有一个全局域（`biz_domain.global = true`）
- 全局域范围=有 CLASSIFY 声明按声明（声明即收窄）、无声明为未被其他域认领的资源类型动态补集（T-PERM-046 定案 2026-09-09；create 可选 global 入口每租户仅一个，20057）
- `DomainClassifyService` 仅负责查询与匹配，不承担全局域创建（biz-domain create 入口）
