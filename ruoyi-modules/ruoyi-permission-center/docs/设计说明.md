...
- **冲突规则与检测**：定义同一资源上互斥的操作对，并在查询/快照组装与治理扫描时检测违规。
...
| 类型定义 (type_definition) | user_type、role_type、resource_type 等 KV 配置 | `/api/perm/type-definitions` |
...
| 资源依赖 (resource_dependency) | 声明式依赖元数据，支持依赖图与按需完整性检查 | `/api/perm/resource-dependencies` |
...
flowchart LR
    ...
    TypeDefinition --> AbstractUser
    TypeDefinition --> AbstractRole
    TypeDefinition --> ResourceEntity
...
- **类型枚举**：user_type、role_type、resource_type 等来自 type_definition（typeKey + typeValue + label）。
...
| 类型配置 | 枚举维护（用户类型/角色类型/资源类型等） | `/api/perm/type-definitions` | list、save、remove |
...
| 资源依赖 | 依赖链配置、依赖图查看、依赖检查 | `/api/perm/resource-dependencies` | list、graph、check、save、remove |
...
| 冲突规则 | 规则维护、全量兼容检测、分页扫描 | `/api/perm/conflict-rules`、`/api/perm/conflict-detection` | list、save、remove、detect |
